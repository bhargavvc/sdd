  ...
  return (
    <div className="flex w-12 flex-col items-center gap-1 border-r border-border bg-sidebar py-3">
      ...
      <div className="mt-auto flex flex-col gap-1">
        <button
          onClick={() => window.dispatchEvent(new CustomEvent("sdd:open-projects"))}
          disabled={isConnecting}
          className={cn(
            "flex h-10 w-10 items-center justify-center rounded-md transition-colors",
            isConnecting
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Stop the SDD web server?</DialogTitle>
            <DialogDescription>
              This will shut down the server process and close this tab. Run{" "}
              <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs">sdd --web</code> again to restart.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
