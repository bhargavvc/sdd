...
if (typeof window !== "undefined") {
  window.addEventListener("sdd:open-file", (e: Event) => {
...
function storageKey(projectCwd: string, root: RootMode): string {
  return `sdd-files-expanded:${root}:${projectCwd}`
}
...
function tabDisplayPath(tab: OpenTab): string {
  return tab.root === "sdd" ? `.sdd/${tab.path}` : tab.path
}
...
export function FilesView() {
  ...
  const handleCopyPath = useCallback((path: string) => {
    const displayPath = activeRoot === "sdd" ? `.sdd/${path}` : path
    ...
    <div className="flex h-full">
      ...
      <div className="flex-shrink-0 border-r border-border overflow-hidden flex flex-col" style={{ width: treeWidth }}>
        ...
        <div className="flex border-b border-border flex-shrink-0">
          <button
            onClick={() => { setLeftPanel("tree"); handleTreeRootChange("sdd") }}
            className={cn(
              "flex-1 px-3 py-2 text-xs font-medium transition-colors",
              leftPanel === "tree" && activeRoot === "sdd"
        ...
        >
          {loading && !treeLoaded ? (
            ...
            <div className="flex items-center justify-center py-8 text-muted-foreground text-xs">
              {activeRoot === "sdd" ? "No .sdd/ files found" : "No files found"}
            </div>
          ) : tree ? (
            <>
