...
import { Sidebar, MilestoneExplorer, CollapsedMilestoneSidebar } from "@/components/sdd/sidebar"
import { ShellTerminal } from "@/components/sdd/shell-terminal"
import { Dashboard } from "@/components/sdd/dashboard"
import { Roadmap } from "@/components/sdd/roadmap"
import { FilesView } from "@/components/sdd/files-view"
import { ActivityView } from "@/components/sdd/activity-view"
import { VisualizerView } from "@/components/sdd/visualizer-view"
import { StatusBar } from "@/components/sdd/status-bar"
import { DualTerminal } from "@/components/sdd/dual-terminal"
import { FocusedPanel } from "@/components/sdd/focused-panel"
import { OnboardingGate } from "@/components/sdd/onboarding-gate"
import { CommandSurface } from "@/components/sdd/command-surface"
...
import { ProjectsPanel, ProjectSelectionGate } from "@/components/sdd/projects-view"
import { UpdateBanner } from "@/components/sdd/update-banner"
...
function viewStorageKey(projectCwd: string): string {
  return `sdd-active-view:${projectCwd}`
}
...
function WorkspaceChrome() {
  ...
  useEffect(() => {
    const restoreTimer = window.setTimeout(() => {
      try {
        const stored = localStorage.getItem("sdd-sidebar-collapsed")
  ...
  useEffect(() => {
    try {
      localStorage.setItem("sdd-sidebar-collapsed", String(sidebarCollapsed))
    } catch {
      // localStorage may be unavailable
    }
  }, [sidebarCollapsed])
  ...
  useEffect(() => {
    ...
    const base = projectLabel ? `SDD - ${projectLabel}` : "SDD"
  ...
  useEffect(() => {
    ...
    window.addEventListener("sdd:open-file", handler)
    return () => window.removeEventListener("sdd:open-file", handler)
  }, [])

  // Listen for cross-component view navigation events (e.g. /sdd visualize dispatch)
  useEffect(() => {
    ...
    window.addEventListener("sdd:navigate-view", handler as EventListener)
    return () => window.removeEventListener("sdd:navigate-view", handler as EventListener)
  }, [handleViewChange])

  // Listen for projects panel toggle (sidebar icon, or programmatic)
  useEffect(() => {
    const handler = () => setProjectsPanelOpen(true)
    window.addEventListener("sdd:open-projects", handler)
    return () => window.removeEventListener("sdd:open-projects", handler)
  }, [])
  ...
  const isWelcomeState =
    ...
    detection != null &&
    detection.kind !== "active-sdd" &&
    detection.kind !== "empty-sdd"
      ...
      <div className="flex h-dvh flex-col items-center justify-center gap-6 bg-background p-8 text-center">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="max-w-sm text-sm text-muted-foreground">
            ...
            <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs">sdd --web</code>.
          </p>
        </div>
      </div>
    )
