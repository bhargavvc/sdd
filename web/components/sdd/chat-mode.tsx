...
/* ─── SDD Action Definitions ─── */

/**
 * Defines every /sdd subcommand available in the chat input bar.
...
const SDD_ACTIONS: SDDActionDef[] = [
  // ── Top 3 (standalone buttons) ──
  { label: "Discuss",   command: "/sdd discuss",   icon: MessageCircle,     description: "Start guided milestone/slice discussion",                    category: "workflow",    disabledDuringAuto: true },
  { label: "Next",      command: "/sdd next",      icon: Play,              description: "Execute next task, then pause",                              category: "workflow" },
  { label: "Auto",      command: "/sdd auto",      icon: Zap,               description: "Run all queued units continuously",                         category: "workflow" },
  // ── Overflow: Workflow ──
  { label: "Stop",      command: "/sdd stop",      icon: Square,            description: "Stop auto-mode gracefully",                                  category: "workflow" },
  { label: "Pause",     command: "/sdd pause",     icon: Pause,             description: "Pause auto-mode (preserves state)",                          category: "workflow" },
  // ── Overflow: Visibility ──
  { label: "Status",    command: "/sdd status",    icon: BarChart3,         description: "Show progress dashboard",                                    category: "visibility" },
  { label: "Visualize", command: "/sdd visualize", icon: LayoutGrid,        description: "Interactive TUI (progress, deps, metrics, timeline)",        category: "visibility" },
  { label: "Queue",     command: "/sdd queue",     icon: ListOrdered,       description: "Show queued/dispatched units and execution order",            category: "visibility" },
  { label: "History",   command: "/sdd history",   icon: History,           description: "View execution history with cost/phase/model details",        category: "visibility" },
  // ── Overflow: Course correction ──
  { label: "Steer",     command: "/sdd steer",     icon: Compass,           description: "Apply user override to active work",                         category: "correction" },
  { label: "Capture",   command: "/sdd capture",   icon: PenLine,           description: "Quick-capture a thought to CAPTURES.md",                     category: "correction" },
  { label: "Triage",    command: "/sdd triage",    icon: Inbox,             description: "Classify and route pending captures",                        category: "correction",  disabledDuringAuto: true },
  { label: "Skip",      command: "/sdd skip",      icon: SkipForward,       description: "Prevent a unit from auto-mode dispatch",                     category: "correction" },
  { label: "Undo",      command: "/sdd undo",      icon: Undo2,             description: "Revert last completed unit",                                 category: "correction" },
  // ── Overflow: Knowledge ──
  { label: "Knowledge", command: "/sdd knowledge", icon: BookOpen,          description: "Add rule, pattern, or lesson to KNOWLEDGE.md",               category: "knowledge" },
  // ── Overflow: Configuration ──
  { label: "Mode",      command: "/sdd mode",      icon: SlidersHorizontal, description: "Set workflow mode (solo/team)",                               category: "config" },
  { label: "Prefs",     command: "/sdd prefs",     icon: Settings,          description: "Manage preferences (global/project)",                        category: "config" },
  // ── Overflow: Maintenance ──
  { label: "Doctor",    command: "/sdd doctor",    icon: Stethoscope,       description: "Diagnose and repair .sdd/ state",                            category: "maintenance" },
  { label: "Export",    command: "/sdd export",    icon: FileOutput,        description: "Export milestone/slice results (JSON or Markdown)",           category: "maintenance" },
  { label: "Cleanup",   command: "/sdd cleanup",   icon: Trash2,            description: "Remove merged branches or snapshots",                        category: "maintenance" },
  { label: "Remote",    command: "/sdd remote",    icon: Globe,             description: "Control remote auto-mode (Slack/Discord)",                    category: "maintenance" },
]

/** Top 3 shown as standalone buttons next to chat input */
const TOP_ACTIONS = SDD_ACTIONS.slice(0, 3)
/** Remaining actions in the overflow menu */
const OVERFLOW_ACTIONS = SDD_ACTIONS.slice(3)

const CATEGORY_LABELS: Record<SDDActionDef["category"], string> = {
  workflow: "Workflow",
...
function groupByCategory(actions: SDDActionDef[]): Array<{ category: SDDActionDef["category"]; label: string; items: SDDActionDef[] }> {
  const seen = new Map<SDDActionDef["category"], SDDActionDef[]>()
 ...
 *
 * All /sdd commands dispatch through the main bridge session.
 ...
 *   - sessionStorage key "sdd-active-view:<cwd>" equals "chat" when this view is active.
...
export function ChatMode({ className }: { className?: string }) {
  const state = useSDDWorkspaceState()
...
function ChatModeHeader({ onPrimaryAction, onSecondaryAction }: ChatModeHeaderProps) {
  ...
  // Derive a short SDD state badge label
  const stateBadge = (() => {
...
/**
 * Renders a SDD arrow-key select prompt as a native clickable list.
 *
...
/**
 * Renders a SDD text prompt as a native labeled input field.
 *
...
/**
 * Renders a SDD password/API-key prompt as a native masked input field.
 *
 ...
 * - Overflow menu (⋯) contains all remaining /sdd subcommands grouped by category
...
function ChatInputBar({
  ...
  connected: boolean
  onOpenAction?: (action: SDDActionDef) => void
}) {
  const autoActive = useSDDWorkspaceState().boot?.auto?.active ?? false
  const [value, setValue] = useState("")
    ...
    <div className="flex flex-1 flex-col items-center justify-center text-center py-16">
      ...
      <div className="mt-3 space-y-1">
        ...
        {showSpinner ? (
          ...
          <p className="max-w-xs text-xs text-muted-foreground">
            Connected — waiting for SDD output…
          </p>
        )}
      </div>
    </div>
...
function InlineUiRequest({ request }: { request: PendingUiRequest }) {
  ...
  const isSubmitting = useSDDWorkspaceState().commandInFlight === "extension_ui_response"
...
export function ChatPane({ className, onOpenAction }: ChatPaneProps) {
  const state = useSDDWorkspaceState()
  const { submitInput, sendCommand, pushChatUserMessage } = useSDDWorkspaceActions()
