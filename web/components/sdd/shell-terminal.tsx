...
import { filterInitialSddHeader } from "@/lib/initial-sdd-header-filter"
...
function TerminalInstance({
  ...
  const initialHeaderSettledRef = useRef(!hideInitialSddHeader)
  ...
  useEffect(() => {
    initialHeaderSettledRef.current = !hideInitialSddHeader
    initialHeaderBufferRef.current = ""
  }, [hideInitialSddHeader, sessionId])
      ...
      es.onmessage = (event) => {
        try {
          ...
          if (msg.type === "connected") {
            ...
            if (hideInitialSddHeader && !initialHeaderSettledRef.current) {
              initialHeaderBufferRef.current += output
              const filtered = filterInitialSddHeader(initialHeaderBufferRef.current)
  ...
  }, [sessionId, command, commandArgs, commandArgsKey, fontSize, hideInitialSddHeader, isDark, projectCwd, sendInput, sendResize])
      ...
      {!hasOutput && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 bg-terminal">
          ...
          <span className="text-xs text-muted-foreground">
            {command ? "Starting SDD…" : "Connecting…"}
          </span>
        </div>
      )}
      <div
...
export function ShellTerminal({
  ...
  const defaultId = sessionPrefix ?? (command ? "sdd-default" : "default")
      ...
      >
        {tabs.map((tab) => (
          <TerminalInstance
            ...
            isDark={isDark}
            fontSize={fontSize}
            hideInitialSddHeader={hideInitialSddHeader}
