...
function bootSeedLines(boot: WorkspaceBootPayload): WorkspaceTerminalLine[] {
  const lines = [
    createTerminalLine("system", `SDD web workspace attached to ${boot.project.cwd}`),
...
function createInitialState(): WorkspaceStoreState {
  return {
    ...
    terminalLines: [createTerminalLine("system", "Preparing the live SDD workspace…")],
    ...
    switch (outcome.kind) {
      ...
      case "view-navigate": {
        ...
        window.dispatchEvent(
          new CustomEvent("sdd:navigate-view", { detail: { view: outcome.view } }),
        )
        return outcome
      }
    }
