 ...
 *
 * The SDD_WEB_AUTH_TOKEN env var is set at server launch. Every /api/* request
...
export function proxy(request: NextRequest): NextResponse | undefined {
  ...
  const expectedToken = process.env.SDD_WEB_AUTH_TOKEN
  if (!expectedToken) {
  ...
  if (origin) {
    const host = process.env.SDD_WEB_HOST || "127.0.0.1"
    const port = process.env.SDD_WEB_PORT || "3000"

    // Default: localhost origin for the launched host:port
    const allowed = new Set([`http://${host}:${port}`])

    // SDD_WEB_ALLOWED_ORIGINS lets users whitelist additional origins for
    // secure tunnel setups (Tailscale Serve, Cloudflare Tunnel, ngrok, etc.)
    const extra = process.env.SDD_WEB_ALLOWED_ORIGINS
    if (extra) {
