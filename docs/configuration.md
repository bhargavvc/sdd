...
**macOS delivery:** SDD uses [`terminal-notifier`](https://github.com/julienXX/terminal-notifier) when available, falling back to `osascript`. We recommend installing `terminal-notifier` for reliable notification delivery:
...
OpenAI service tier preference for supported models. Toggle with `/sdd fast`.
...
# Service tier
service_tier: priority         # "priority" or "flex" (for /sdd fast)
