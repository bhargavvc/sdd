...
SDD uses a three-stage promotion pipeline that automatically moves merged PRs through **Dev → Test → Prod** environments using npm dist-tags.
...
```bash
# Test candidate
docker run --rm -v $(pwd):/workspace ghcr.io/bhargavvc/sdd-pi:next --version

# Stable
docker run --rm -v $(pwd):/workspace ghcr.io/bhargavvc/sdd-pi:latest --version
```
...
# Roll back Docker
docker pull ghcr.io/bhargavvc/sdd-pi:<previous-good-version>
docker tag ghcr.io/bhargavvc/sdd-pi:<previous-good-version> ghcr.io/bhargavvc/sdd-pi:latest
...
| GHCR | Enabled for the `bhargavvc` org |

### Docker Images

| Image | Base | Purpose | Tags |
|-------|------|---------|------|
| `ghcr.io/bhargavvc/sdd-ci-builder` | `node:24-bookworm` | CI build environment with Rust toolchain | `:latest`, `:<date>` |
| `ghcr.io/bhargavvc/sdd-pi` | `node:24-slim` | User-facing runtime | `:latest`, `:next`, `:v<version>` |
