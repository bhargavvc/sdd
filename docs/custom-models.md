...
Add custom providers and models (Ollama, vLLM, LM Studio, proxies) via `~/.sdd/agent/models.json`.
...
Some OpenAI-compatible servers do not understand the `developer` role used for reasoning-capable models. For those providers, set `compat.supportsDeveloperRole` to `false` so SDD sends the system prompt as a `system` message instead. If the server also does not support `reasoning_effort`, set `compat.supportsReasoningEffort` to `false` too.
...
| `reasoningEffortMap` | Map SDD thinking levels to provider-specific `reasoning_effort` values |
