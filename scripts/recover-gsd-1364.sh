...
#   4. Removes the bad ".gsd" line from .gitignore (if .sdd/ is tracked)
...
# Look for a bare ".gsd" line (not a comment, not a sub-path like .sdd/)
GSD_IGNORE_LINE=""
if [[ -f "$GITIGNORE" ]]; then
...
if $GSD_IS_SYMLINK; then
  ...
  if [[ -z "$GSD_IGNORE_LINE" ]]; then
    warn '".gsd" missing from .gitignore — will add (migration complete, .sdd/ is external).'
  else
...
if $GSD_IS_SYMLINK; then
  ...
  if [[ -z "$GSD_IGNORE_LINE" ]]; then
    info 'Adding ".gsd" to .gitignore (migration complete — .sdd/ is external state)...'
    if $DRY_RUN; then
  ...
  if [[ -z "$GSD_IGNORE_LINE" ]]; then
    ...
    if $DRY_RUN; then
      ...
      # Remove the exact line ".gsd" (not comments, not .sdd/ subdirs)
