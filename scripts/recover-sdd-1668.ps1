# recover-sdd-1668.ps1 — Recovery script for issue #1668 (Windows)
#
...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File scripts\recover-sdd-1668.ps1 [-MilestoneId <ID>] [-DryRun] [-Auto]
#
# Options:
#   -MilestoneId <ID>   SDD milestone ID (e.g. M001-g2nalq).
...
if (-not $reflogFoundSha) {
    ...
    foreach ($sha in $unreachableCommits) {
        ...
        if (($commitMsg + $commitBody) -match 'milestone/|complete-milestone|SDD|slice') { $score += 20 }
...
Write-Host ""
Write-Host "Note: update SDD to v2.40.1+ to prevent this from recurring." -ForegroundColor DarkGray
Write-Host "      PR: https://github.com/sdd-build/sdd-2/pull/1669" -ForegroundColor DarkGray
Write-Host ""
