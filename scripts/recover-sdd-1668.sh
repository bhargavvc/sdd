#!/usr/bin/env bash
# recover-sdd-1668.sh — Recovery script for issue #1668 (Linux / macOS)
#
# SDD v2.39.x deleted the milestone branch and worktree directory when a
...
#   3. Ranks candidates by recency and SDD commit message patterns
#   4. Creates a recovery branch at the identified commit
#   5. Reports what was found and how to complete the merge manually
#
# Usage:
#   bash scripts/recover-sdd-1668.sh [--milestone <ID>] [--dry-run] [--auto]
#
# Options:
#   --milestone <ID>   SDD milestone ID (e.g. M001-g2nalq).
...
#
# Affected versions: SDD 2.39.x
# Fixed in: SDD 2.40.1 (PR #1669)
...
if [[ -z "$REFLOG_FOUND_SHA" ]]; then
  ...
  # Score each unreachable commit — rank by recency and SDD message patterns.
  # SDD milestone commits look like: "feat(M001-g2nalq): <title>"
  ...
  while IFS=$'\t' read -r sha commit_ts commit_date_hr commit_msg; do
    ...
    if echo "$commit_msg" | grep -qiE 'milestone/|complete-milestone|SDD|slice'; then
      SCORE=$((SCORE + 20))
    fi
  ...
  info "Top candidates (scored by recency and SDD message patterns):"
  echo ""
  NUM=1
  SORTED_IDXS=()
...
echo ""
echo -e "${DIM}Note: update SDD to v2.40.1+ to prevent this from recurring.${RESET}"
echo "      PR: https://github.com/sdd-build/sdd-2/pull/1669"
echo ""
