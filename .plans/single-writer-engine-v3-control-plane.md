...
in-process `workflow-logger` buffer to a persistent `.sdd/audit-log.jsonl`
...
**File:** `src/resources/extensions/sdd/tools/complete-task.ts`
...
**File:** `src/resources/extensions/sdd/tools/complete-slice.ts`
...
**File:** `src/resources/extensions/sdd/tools/complete-milestone.ts`
...
**File:** `src/resources/extensions/sdd/tools/plan-task.ts`
...
**File:** `src/resources/extensions/sdd/tools/plan-slice.ts`
...
**File:** `src/resources/extensions/sdd/tools/plan-milestone.ts`
...
**File:** `src/resources/extensions/sdd/tools/reassess-roadmap.ts`
...
**File:** `src/resources/extensions/sdd/tools/replan-slice.ts`
...
**File:** `src/resources/extensions/sdd/workflow-events.ts`
...
**Files:** All 8 handlers in `src/resources/extensions/sdd/tools/`
...
#### S2-T3: Persist `workflow-logger` to `.sdd/audit-log.jsonl`

**File:** `src/resources/extensions/sdd/workflow-logger.ts`
...
to `.sdd/audit-log.jsonl` (using `appendFileSync`). This requires the basePath
...
**New file:** `src/resources/extensions/sdd/unit-ownership.ts`

Lightweight JSON file at `.sdd/unit-claims.json` mapping unit IDs to agent names:
```json
{
...
If `actor_name` is provided AND `.sdd/unit-claims.json` exists AND the unit is claimed:
...
1. **Ownership: opt-in** — enforced only when `.sdd/unit-claims.json` exists. Zero breaking change for existing workflows; teams adopt incrementally.
