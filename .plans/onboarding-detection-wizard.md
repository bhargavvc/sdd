...
> **Phase 2 (Global Setup refactor)**: Deferred — `/sdd setup` provides routing to existing commands for now
...
Neither detects v1 `.planning/` directories, explains what SDD is, offers project-level configuration, or helps returning users entering a new folder.

## Design Decisions

### Q1: Should bare `/sdd` show a menu or auto-start?
...
| No `.sdd/`, has `.planning/` | **Migration offer** → `/sdd migrate` or skip |
| Has `.sdd/`, no milestones | Current flow (discuss prompt) |
| Has `.sdd/`, has milestones | Current flow (smart entry / auto resume) |
| First-ever SDD launch (no `~/.sdd/`) | **Global setup** first, then project init |

### Q2: Should there be an onboarding wizard?

**Answer: Yes — two-tier wizard.**

- **Global wizard** (`/sdd setup`) — runs once per machine. Handles: LLM auth (absorb current `onboarding.ts`), global preferences (default model, mode, timeout defaults), tool keys, remote questions.
- **Project wizard** (`/sdd init`) — runs once per project folder. Handles: project-type detection, preferences template, git init, `.gitignore`, optional CONTEXT.md seeding.

Both wizards should be:
- Skippable at every step
- Re-runnable (`/sdd setup` and `/sdd init` work any time)
...
New subcommands: `/sdd init`, `/sdd setup`. Existing `/sdd migrate` stays.

### Modified: `src/onboarding.ts`

Refactored to be callable from both pre-TUI boot and `/sdd setup`.
...
interface ProjectDetection {
  ...
  /** v2 details (if state === 'v2-sdd' or 'v2-sdd-empty') */
  v2?: {
...
## Phase 2: Global Setup Wizard (`/sdd setup`)
...
### Task 2.2: `/sdd setup` command handler

```
/sdd setup          → full wizard (all steps)
/sdd setup llm      → just LLM auth
/sdd setup search   → just web search
/sdd setup remote   → just remote questions
/sdd setup keys     → just tool keys
/sdd setup prefs    → just global preferences
```
...
│  ✓ Remote questions: Discord #sdd-bot        │
...
## Phase 3: Project Init Wizard (`/sdd init`)
...
#### Tier 3: Don't ask at init (defer to `/sdd prefs project`)
...
Step 5: Verification Commands (auto-populated from detection)
   "How should SDD verify code changes?"
...
Step 7: Project Instructions (optional, skippable)
   "Any rules SDD should follow for this project?"
...
Step 9: Bootstrap .sdd/ structure
   ...
   - Commits "chore: init sdd" (if commit_docs enabled)
...
```
┌ SDD — Legacy Project Detected ────────────────┐
│                                               │
│  Found .planning/ directory (SDD v1 format)   │
│  3 phases, 12 tasks detected                  │
│                                               │
│  > Migrate to SDD v2    (recommended)         │
│    Start fresh                                │
│    Cancel                                     │
│                                               │
└───────────────────────────────────────────────┘
...
If `.sdd/` already exists when `/sdd init` is run:
...
- Re-run `/sdd init` on existing project → safe behavior

### Task 5.3: Global setup tests

- `/sdd setup` from command handler
- Individual sub-steps (`/sdd setup llm`, etc.)
...
| `src/resources/extensions/sdd/commands.ts` | **MODIFY** | Add `/sdd init` and `/sdd setup` subcommands |
| Tests (TBD paths) | **NEW** | Detection, init, setup tests |

## Open Questions for Discussion

1. **Should `/sdd init` auto-transition to the discuss prompt?** Or end with "Run /sdd to start"? Auto-transition is more fluid but might feel jarring after a wizard.
...
3. **Should `/sdd setup` be accessible outside the TUI?** e.g. `sdd setup` from the shell before launching. Currently `onboarding.ts` handles this but it's limited.

4. **Migration: should we auto-detect `.planning/` in parent directories?** Some users might run SDD from a subdirectory while `.planning/` is at the repo root.
