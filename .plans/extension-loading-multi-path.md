...
SDD-2 has a well-structured extension system with three discovery paths (bundled, global/community, project-local) that are **already wired up** through pi's `DefaultPackageManager.addAutoDiscoveredResources()`. However, two critical gaps remain:
...
2. The SDD extension registry (enable/disable) only applies to **bundled** extensions — community extensions bypass it entirely
...
SDD loader.ts
  ...
  → filter by SDD registry (isExtensionEnabled)
  → set SDD_BUNDLED_EXTENSION_PATHS env var
      ↓
DefaultResourceLoader.reload()
  → packageManager.resolve()
    → addAutoDiscoveredResources()
      → project: cwd/.sdd/extensions/     (CONFIG_DIR_NAME = ".sdd")
      → global:  ~/.sdd/agent/extensions/  (includes synced bundled)
...
| File | Role |
|------|------|
| `src/loader.ts` (lines 146-161) | SDD startup — bundled discovery + registry filter |
...
| `packages/pi-coding-agent/src/core/package-manager.ts` (lines 1585-1700) | `addAutoDiscoveredResources()` — auto-discovers from .sdd/ dirs |
...
**Consideration:** `sortExtensionPaths` lives in `src/` (SDD side), not in `packages/pi-coding-agent/`. Need to either:
- (a) Move it into pi-coding-agent as a shared utility, OR
- (b) Import it cross-package (already done for other SDD→pi imports), OR
- (c) Call it on the SDD side before paths reach pi — harder since auto-discovered paths are added inside pi's package manager
...
### Task 2: Apply SDD registry to community extensions

**What:** When `buildResourceLoader()` in `src/resource-loader.ts` constructs the DefaultResourceLoader, also discover and filter community extensions from `~/.sdd/agent/extensions/` through the SDD registry — same as it already does for `~/.pi/agent/extensions/` paths.
...
- Discover extensions in `~/.sdd/agent/extensions/` that are NOT bundled
...
**Alternative approach:** Hook into `addAutoDiscoveredResources` or the `addResource` call to check the SDD registry. This might be cleaner since the auto-discovery already happens inside pi's package manager.

### Task 3: Emit sort warnings as diagnostics

**What:** Surface dependency warnings (missing deps, cycles) through SDD's diagnostic system so users see them.

**Where:** Wherever the sort is invoked from Task 1.

**Format:**
```
⚠ Extension 'sdd-watch' declares dependency 'sdd' which is not installed — loading anyway
...
- **Registry community test:** Drop a community extension in `~/.sdd/agent/extensions/`, run `sdd extensions disable <id>`, verify it doesn't load.
...
**Subagent extension forwarding:** Update `src/resources/extensions/subagent/index.ts` to forward ALL extension paths (not just bundled) to child processes. May need a second env var like `SDD_COMMUNITY_EXTENSION_PATHS` or consolidate into `SDD_EXTENSION_PATHS`.

---

## Open Questions

1. **Where should `sortExtensionPaths` live?** Currently in `src/` (SDD side). Needs to be callable from pi's resource-loader. Options: move to pi, keep and import cross-package, or duplicate.
...
3. **Project-local trust:** The TOFU model in the dead `discoverAndLoadExtensions()` never runs. Should `addAutoDiscoveredResources` also gate project-local extensions behind trust? Or is `.sdd/extensions/` in your own project always trusted?
