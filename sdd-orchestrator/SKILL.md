...
description: >
  Build software products autonomously via SDD headless mode. Handles the full
  ...
  "create a project", "run sdd", "check build status", or any task that
...
<objective>
You are an autonomous agent that builds software by orchestrating SDD as a subprocess.
SDD is a headless CLI that plans, codes, tests, and ships software from a spec.
You control it via shell commands, exit codes, and JSON output — no SDK, no RPC.
</objective>

<mental_model>
SDD headless is a subprocess you launch and monitor. Think of it like a junior developer
you hand a spec to:

1. You write the spec (what to build)
2. You launch the build (`sdd headless ... new-milestone --context spec.md --auto`)
...
You never write application code yourself — SDD does that.
</mental_model>

<critical_rules>
- **Flags before command.** `sdd headless [--flags] [command] [args]`. Flags after the command are ignored.
...
- **One project directory per build.** Each SDD project needs its own directory with a `.sdd/` folder.
</critical_rules>
...
Build a ...
EOF
sdd headless --output-format json --context spec.md new-milestone --auto 2>/dev/null
```

**Check project state (instant, free):**
```bash
cd /path/to/project
sdd headless query | jq '{phase: .state.phase, progress: .state.progress, cost: .cost.total}'
```
...
| `1`  | Error or timeout | Inspect stderr, check `.sdd/STATE.md`, retry or escalate |
...
<project_structure>
SDD creates and manages all state in `.sdd/`:
```
.sdd/
...
State is derived from files on disk — checkboxes in ROADMAP.md and PLAN.md are the source of truth for completion. You never need to edit these files. SDD manages them. But you can read them to understand progress.
</project_structure>
...
```bash
sdd headless --answers answers.json --output-format json auto 2>/dev/null
```
