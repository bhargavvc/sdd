...
When using `--output-format json`, SDD collects events silently and emits a single `HeadlessJsonResult` JSON object to stdout at process exit. This is the structured result for orchestrator decision-making.
...
| `phase` | `string \| undefined` | Current SDD phase at session end (e.g. `"executing"`, `"blocked"`, `"complete"`). |
...
sdd headless --resume "$SESSION_ID" --output-format json next 2>/dev/null
```

### Artifact Collection

```bash
RESULT=$(sdd headless --output-format json auto 2>/dev/null)
...
```json
{
  ...
  "nextAction": "dispatch",
  "artifacts": [
    ".sdd/milestones/M001/slices/S01/tasks/T01-SUMMARY.md"
  ],
  "commits": [
    "a1b2c3d"
  ]
...
sdd headless query | jq '{phase: .state.phase, progress: .state.progress, totalCost: .cost.total}'
```
