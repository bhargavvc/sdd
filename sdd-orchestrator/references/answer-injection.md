...
sdd headless --answers answers.json new-milestone --context spec.md --auto
```
...
Secrets are injected as environment variables into the SDD child process:

1. The orchestrator passes the answer file via `--answers`
2. SDD reads the file and sets secret values as env vars in the child process
...
1. **Observe** — SDD monitors `tool_execution_start` events for `ask_user_questions` to extract question metadata (ID, options, allowMultiple)
...
# Parse result
RESULT=$(sdd headless --answers answers.json --output-format json next 2>/dev/null)
