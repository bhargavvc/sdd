...
/**
 * TreeDataProvider that shows real-time tool executions from the SDD agent.
 * Listens to tool_execution_start/end and agent_start/end events.
 */
export class SddActivityFeedProvider implements vscode.TreeDataProvider<ActivityItem>, vscode.Disposable {
	public static readonly viewId = "sdd-activity";
	...
	constructor(private readonly client: SddClient) {
		this.maxItems = vscode.workspace.getConfiguration("sdd").get<number>("activityFeedMaxItems", 100);
		...
		this.disposables.push(
			...
			vscode.workspace.onDidChangeConfiguration((e) => {
				if (e.affectsConfiguration("sdd.activityFeedMaxItems")) {
					this.maxItems = vscode.workspace.getConfiguration("sdd").get<number>("activityFeedMaxItems", 100);
				}
			}),
		);
