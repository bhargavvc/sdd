...
import {
	...
	SDD_APPROVAL_CONFIG_KEY,
	SDD_APPROVAL_CONFIG_PATH,
	SDD_APPROVAL_CONFIG_SECTION,
	nextApprovalMode,
...
export class SddPermissionManager implements vscode.Disposable {
	...
	constructor(private readonly client: SddClient) {
		...
		this._mode = vscode.workspace.getConfiguration(SDD_APPROVAL_CONFIG_SECTION).get<ApprovalMode>(SDD_APPROVAL_CONFIG_KEY, "auto-approve");

		this.disposables.push(
			this._onModeChange,
			vscode.workspace.onDidChangeConfiguration((e) => {
				if (e.affectsConfiguration(SDD_APPROVAL_CONFIG_PATH)) {
					this._mode = vscode.workspace.getConfiguration(SDD_APPROVAL_CONFIG_SECTION).get<ApprovalMode>(SDD_APPROVAL_CONFIG_KEY, "auto-approve");
	...
	async cycleMode(): Promise<void> {
		...
		await vscode.workspace.getConfiguration(SDD_APPROVAL_CONFIG_SECTION).update(SDD_APPROVAL_CONFIG_KEY, this._mode, vscode.ConfigurationTarget.Workspace);
		this._onModeChange.fire(this._mode);
