 ...
 * current SDD session using the get_messages RPC call. Shows tool calls,
 * thinking blocks, search/filter, and fork-from-here actions.
 */
export class SddConversationHistoryPanel implements vscode.Disposable {
	private static currentPanel: SddConversationHistoryPanel | undefined;
	...
	static createOrShow(
		...
		client: SddClient,
	): SddConversationHistoryPanel {
		const column = vscode.window.activeTextEditor?.viewColumn ?? vscode.ViewColumn.One;

		if (SddConversationHistoryPanel.currentPanel) {
			SddConversationHistoryPanel.currentPanel.panel.reveal(column);
			void SddConversationHistoryPanel.currentPanel.refresh();
			return SddConversationHistoryPanel.currentPanel;
		}

		const panel = vscode.window.createWebviewPanel(
			"sdd-history",
			"SDD Conversation History",
			column,
			{
				enableScripts: true,
		...
		SddConversationHistoryPanel.currentPanel = new SddConversationHistoryPanel(
			panel,
			extensionUri,
			client,
		);
	...
	async refresh(): Promise<void> {
		if (!this.client.isConnected) {
			this.panel.webview.html = this.getHtml([], "Not connected to SDD agent.");
			return;
		}
	...
	dispose(): void {
		SddConversationHistoryPanel.currentPanel = undefined;
		this.panel.dispose();
	...
	private getHtml(messages: ConversationMessage[], errorMessage?: string): string {
		...
		const renderedMessages = visibleMessages
			.map((msg, idx) => {
				...
				<div class="role-row">
					<span class="role">${isUser ? "You" : "SDD"}</span>
					${forkBtn}
				</div>
