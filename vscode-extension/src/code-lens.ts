...
/**
 * CodeLensProvider that adds an "Ask SDD" lens above named function and class
 * declarations. Clicking the lens sends a brief explanation request to the SDD
 * agent for that specific symbol.
 */
export class SddCodeLensProvider implements vscode.CodeLensProvider, vscode.Disposable {
	...
	constructor(private readonly client: SddClient) {
		this.disposables.push(
			...
			vscode.workspace.onDidChangeConfiguration((e) => {
				if (e.affectsConfiguration("sdd.codeLens")) {
	...
	provideCodeLenses(
		...
		if (!vscode.workspace.getConfiguration("sdd").get<boolean>("codeLens", true)) {
			return lenses;
		}
				...
				if (match && match[1] && !seen.has(i)) {
					...
					lenses.push(
						new vscode.CodeLens(range, {
							title: "$(hubot) Ask SDD",
							tooltip: `Ask SDD to explain ${symbolName}`,
							command: "sdd.askAboutSymbol",
							arguments: args,
						}),
						new vscode.CodeLens(range, {
							title: "$(pencil) Refactor",
							tooltip: `Refactor ${symbolName}`,
							command: "sdd.refactorSymbol",
							arguments: args,
						}),
						new vscode.CodeLens(range, {
							title: "$(bug) Find Bugs",
							tooltip: `Review ${symbolName} for bugs`,
							command: "sdd.findBugsSymbol",
							arguments: args,
						}),
						new vscode.CodeLens(range, {
							title: "$(beaker) Tests",
							tooltip: `Generate tests for ${symbolName}`,
							command: "sdd.generateTestsSymbol",
							arguments: args,
						}),
					);
				}
