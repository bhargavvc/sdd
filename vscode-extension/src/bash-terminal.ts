...
/**
 * Routes the SDD agent's Bash tool output to a dedicated VS Code terminal panel.
 * Shows streaming output from tool_execution_update events in real time.
 */
export class SddBashTerminal implements vscode.Disposable {
	private terminal: vscode.Terminal | undefined;
	private writeEmitter: vscode.EventEmitter<string> | undefined;
	private disposables: vscode.Disposable[] = [];

	constructor(client: SddClient) {
	...
	private getOrCreateTerminal(): { terminal: vscode.Terminal; writeEmitter: vscode.EventEmitter<string> } {
		if (!this.terminal || this.terminal.exitStatus !== undefined) {
			...
			this.terminal = vscode.window.createTerminal({ name: "SDD Agent", pty });
		}
