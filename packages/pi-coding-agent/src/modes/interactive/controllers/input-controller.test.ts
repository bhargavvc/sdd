...
test("input-controller: extension slash commands fall through to session.prompt", async () => {
	const { host, prompted, errors, history } = createHost({ knownSlashCommands: ["sdd"] });

	await host.defaultEditor.onSubmit("/sdd help");

	assert.deepEqual(prompted, ["/sdd help"], "known extension slash commands should reach session.prompt");
	assert.deepEqual(errors, [], "known extension slash commands should not show unknown-command errors");
	assert.deepEqual(history, ["/sdd help"], "known extension slash commands should still be added to history");
});
