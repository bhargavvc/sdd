...
export class DiscordBot {
  ...
  /**
   * Set the EventBridge reference so the bot can dispatch /sdd-verbose commands.
    ...
    switch (commandName) {
      case 'sdd-status': {
        ...
        interaction.reply({ content, ephemeral: true }).catch((err) => {
          this.logger.warn('sdd-status reply failed', {
            error: err instanceof Error ? err.message : String(err),
          });
        });
        break;
      ...
      case 'sdd-start':
        this.handleGsdStart(interaction).catch((err) => {
          this.logger.warn('sdd-start handler error', {
            error: err instanceof Error ? err.message : String(err),
          });
        });
        break;
      case 'sdd-stop':
        this.handleGsdStop(interaction).catch((err) => {
          this.logger.warn('sdd-stop handler error', {
            error: err instanceof Error ? err.message : String(err),
          });
        });
        break;
      case 'sdd-verbose': {
        if (!this.eventBridge) {
          interaction.reply({ content: 'Event bridge not available.', ephemeral: true }).catch((err) => {
            this.logger.warn('sdd-verbose reply failed', {
              error: err instanceof Error ? err.message : String(err),
            });
          });
          break;
  ...
  private async handleGsdStart(interaction: import('discord.js').ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      this.logger.error('sdd-start: scan failed', {
        error: err instanceof Error ? err.message : String(err),
      });
    ...
    try {
      ...
      this.logger.info('sdd-start: project selected', { projectPath });

      // Defer the update immediately — startSession can take 10-30s to spawn the SDD process,
      ...
      try {
        ...
        this.logger.error('sdd-start: startSession failed', { error: errMsg, projectPath });
        await interaction.editReply({
  ...
  private async handleGsdStop(interaction: import('discord.js').ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      this.logger.info('sdd-stop: session selected', { sessionId });
      ...
      try {
        ...
        const errMsg = err instanceof Error ? err.message : String(err);
        this.logger.error('sdd-stop: cancelSession failed', { error: errMsg, sessionId });
        await collected.update({
      ...
      // Timeout or other collector error
      this.logger.info('sdd-stop: selection timed out');
      await interaction.editReply({ content: 'Selection timed out.', components: [] });
    }
  }
}
