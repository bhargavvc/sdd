...
<!-- NOTE: Park, unpark, reorder, discard, and dependency-update operations are intentionally
     file-based. No sdd_* tool API exists for these milestone-lifecycle mutations yet.
     The single-writer DB tools (sdd_plan_milestone, sdd_complete_milestone, etc.) own
     create and complete; queue management is file-driven until tool support is added. -->

### Reorder milestones
Change execution order of pending/active milestones. Write `.sdd/QUEUE-ORDER.json`:
```json
...
### Add a new milestone
Use the `sdd_milestone_generate_id` tool to get the next ID, then call `sdd_summary_save` with `milestone_id: {ID}`, `artifact_type: "CONTEXT"`, and the scope/goals/success criteria as `content` — the tool writes the context file to disk and persists to DB. Update QUEUE-ORDER.json to place it at the desired position.
