//! SDD `.sdd/` directory file parser.
//!
//! Parses markdown files containing YAML-like frontmatter, section headings,
//! and structured content used by SDD's planning system (roadmaps, plans,
...
//! - `batchParseSddFiles`: walk a `.sdd/` tree and parse all `.md` files in parallel
...
/// A single parsed SDD file from batch parsing.
#[napi(object)]
pub struct ParsedSddFile {
...
#[napi(js_name = "batchParseSddFiles")]
pub fn batch_parse_sdd_files(directory: String) -> Result<BatchParseResult> {
    ...
    for (path, content) in &file_contents {
        ...
        parsed_files.push(ParsedSddFile {
            path: path.clone(),
            metadata,
            body: body.to_string(),
...
#[napi(js_name = "scanSddTree")]
pub fn scan_sdd_tree(directory: String) -> Result<Vec<SddTreeEntry>> {
...
fn collect_tree_entries(base: &Path, dir: &Path, entries: &mut Vec<SddTreeEntry>) -> Result<()> {
