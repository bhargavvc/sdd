const SUBCOMMAND_HELP: Record<string, string> = {
  ...
  install: [
    'Usage: sdd install <source> [-l, --local]',
    '',
    'Install a package/extension source and run post-install validation (dependency checks, setup).',
    '',
    'Examples:',
    '  sdd install npm:@foo/bar',
    '  sdd install git:github.com/user/repo',
    '  sdd install https://github.com/user/repo',
    '  sdd install ./local/path',
  ].join('\n'),

  remove: [
    'Usage: sdd remove <source> [-l, --local]',
    '',
  ...
  headless: [
    ...
    '  sdd headless --output-format json auto           Structured JSON result on stdout',
    '  sdd headless --json status                      Machine-readable JSONL stream',
    '  sdd headless --timeout 60000                    With 1-minute timeout',
    '  sdd headless --bare auto                        Minimal context (CI/ecosystem use)',
    '  sdd headless --resume abc123 auto               Resume a prior session',
