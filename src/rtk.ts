...
export const SDD_RTK_DISABLED_ENV = "SDD_RTK_DISABLED";
export const SDD_SKIP_RTK_INSTALL_ENV = "SDD_SKIP_RTK_INSTALL";
export const SDD_RTK_PATH_ENV = "SDD_RTK_PATH";
...
export function isRtkEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return !isTruthy(env[SDD_RTK_DISABLED_ENV]);
}

function resolveAppRoot(env: NodeJS.ProcessEnv = process.env): string {
  return env.SDD_HOME || join(osHomedir(), ".sdd");
}
...
export function resolveRtkBinaryPath(options: ResolveRtkBinaryPathOptions = {}): string | null {
  ...
  const explicitPath = env[SDD_RTK_PATH_ENV];
...
export async function ensureRtkAvailable(options: EnsureRtkOptions = {}): Promise<EnsureRtkResult> {
  const env = options.env ?? process.env;
  if (!isRtkEnabled(env)) {
    return { enabled: false, supported: true, available: false, source: "disabled", reason: `${SDD_RTK_DISABLED_ENV} is set` };
  }
  if (isTruthy(env[SDD_SKIP_RTK_INSTALL_ENV])) {
    const configuredPath = env[SDD_RTK_PATH_ENV];
    if (configuredPath && existsSync(configuredPath)) {
      return { enabled: true, supported: true, available: true, source: "managed", binaryPath: configuredPath };
    }
    return { enabled: true, supported: true, available: false, source: "missing", reason: `${SDD_SKIP_RTK_INSTALL_ENV} is set` };
  }
  ...
  try {
    ...
    const checksumsResponse = await fetch(checksumsUrl, { headers: { "User-Agent": "sdd-pi-rtk" } });
...
export async function bootstrapRtk(options: EnsureRtkOptions = {}): Promise<EnsureRtkResult> {
  ...
  if (result.binaryPath) {
    process.env[SDD_RTK_PATH_ENV] = result.binaryPath;
  }
  return result;
}
