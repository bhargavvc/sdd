 ...
 * with a broken import to persist at ~/.sdd/agent/extensions/).
 */
...
test("resource manifest includes contentHash", async (t) => {
  ...
  assert.equal(read.sddVersion, "2.28.0");
...
test("missing contentHash in manifest triggers re-sync (upgrade path)", () => {
  ...
  const shouldSkip = oldManifest.sddVersion === "2.28.0"
...
test("matching contentHash skips re-sync", () => {
  ...
  const shouldSkip = manifest.sddVersion === "2.28.0"
