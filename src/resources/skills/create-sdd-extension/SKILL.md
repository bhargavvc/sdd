---
name: create-sdd-extension
description: Create, debug, and iterate on SDD extensions (TypeScript modules that add tools, commands, event hooks, custom UI, and providers to SDD). Use when asked to build an extension, add a tool the LLM can call, register a slash command, hook into SDD events, create custom TUI components, or modify SDD behavior. Triggers on "create extension", "build extension", "add a tool", "register command", "hook into sdd", "custom tool", "sdd plugin", "sdd extension".
---
...
- Extension loads on SDD startup or `/reload` without errors
