...
```bash
# List open PRs
gh pr list -R sdd-build/sdd-2

# View PR details
gh pr view <number> -R sdd-build/sdd-2

# Check PR CI status
gh pr checks <number> -R sdd-build/sdd-2

# Create PR
gh pr create -R sdd-build/sdd-2 --title "title" --body "body"

# View PR comments
gh api repos/sdd-build/sdd-2/pulls/<number>/comments
```
...
```bash
# List issues
gh issue list -R sdd-build/sdd-2

# List by label
gh issue list -R sdd-build/sdd-2 --label "priority:p1" --state open
...
gh issue create -R sdd-build/sdd-2 \
  --title "feat: add feature X" \
  --label "priority:p1" \
  --milestone "v1.0"

# View issue
gh issue view <number> -R sdd-build/sdd-2

# Close issue with comment
gh issue close <number> -R sdd-build/sdd-2 --comment "Implemented in PR #N"

# Edit labels on issue
gh issue edit <number> -R sdd-build/sdd-2 \
...
ISSUE_URL=$(gh issue create -R sdd-build/sdd-2 \
  --title "..." --body "...")

# Step 2: Set the issue type via GraphQL
ISSUE_NUM=$(echo "$ISSUE_URL" | grep -oE '[0-9]+$')
ISSUE_ID=$(gh api graphql -f query='{ repository(owner:"sdd-build",name:"sdd-2") { issue(number:'"$ISSUE_NUM"') { id } } }' --jq '.data.repository.issue.id')
TYPE_ID=$(gh api graphql -f query='{ repository(owner:"sdd-build",name:"sdd-2") { issueTypes(first:20) { nodes { id name } } } }' --jq '.data.repository.issueTypes.nodes[] | select(.name=="Bug") | .id')
...
```bash
# List all labels
gh label list -R sdd-build/sdd-2

# Create label
gh label create "priority:p1" --color "E99695" \
  --description "High priority" -R sdd-build/sdd-2
```
...
```bash
# List projects
gh project list --owner sdd-build

# Create project
gh project create --owner sdd-build --title "sdd-2 Backlog"

# Add issue to project
gh project item-add 1 --owner sdd-build \
  --url https://github.com/sdd-build/sdd-2/issues/42
```
...
```bash
# List milestones
gh api repos/sdd-build/sdd-2/milestones
...
gh api repos/sdd-build/sdd-2/issues/42 \
...
```bash
# List recent runs
gh run list -R sdd-build/sdd-2 --limit 5

# View specific run
gh run view <run-id> -R sdd-build/sdd-2

# View failed job logs
gh run view <run-id> -R sdd-build/sdd-2 --log-failed
```
...
```bash
# List releases
gh release list -R sdd-build/sdd-2

# View latest release
gh release view --repo sdd-build/sdd-2
```
...
```bash
# GET request
gh api repos/sdd-build/sdd-2

# POST with fields
gh api repos/sdd-build/sdd-2/issues -f title="Bug" -f body="Details"

# GraphQL
gh api graphql -f query='{ viewer { login } }'

# Paginated results
gh api repos/sdd-build/sdd-2/contributors --paginate
```
...
```bash
# Clone
gh repo clone sdd-build/sdd-2

# View repo info
gh repo view -R sdd-build/sdd-2
```
...
```bash
# JSON output
gh pr list -R sdd-build/sdd-2 --json number,title,state

# JQ filtering
gh pr list -R sdd-build/sdd-2 --json number,title --jq '.[].title'

# Template formatting
gh pr list -R sdd-build/sdd-2 --json number,title \
