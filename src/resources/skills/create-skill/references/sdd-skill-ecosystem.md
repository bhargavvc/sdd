<overview>
SDD-specific skill ecosystem details: directory conventions, discovery mechanics, telemetry, and health monitoring. Read this reference when creating or auditing skills within SDD.
</overview>
...
- Available in every SDD session regardless of working directory
- Installed by default or by the user

**Project-scope (local):** `.pi/agent/skills/`
- Available only when SDD runs inside the project directory
- The project-local directory uses `.pi` (inherited from the pi base), **not** `.sdd`
...
Telemetry data is stored in `~/.sdd/metrics.json` alongside other SDD metrics.
</skill_telemetry>
