 ...
 *
 * Staleness detection for managed SDD resources and utilities
 ...
 */
function isManifestWithVersion(data: unknown): data is { sddVersion: string } {
  return data !== null && typeof data === "object" && "sddVersion" in data! && typeof (data as Record<string, unknown>).sddVersion === "string";
}

export function readResourceVersion(): string | null {
  const agentDir = process.env.SDD_CODING_AGENT_DIR || join(sddHome, "agent");
  const manifestPath = join(agentDir, "managed-resources.json");
  const manifest = loadJsonFileOrNull(manifestPath, isManifestWithVersion);
  return manifest?.sddVersion ?? null;
}
