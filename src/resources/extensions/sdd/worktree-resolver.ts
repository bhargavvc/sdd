...
export class WorktreeResolver {
  ...
  private rebuildGitService(): void {
    const gitConfig =
      this.deps.loadEffectiveSDDPreferences()?.preferences?.git ?? {};
  ...
  private _mergeWorktreeMode(milestoneId: string, ctx: NotifyCtx): void {
    ...
    try {
      ...
      // silently or .sdd/ is not symlinked. Without the fallback, a missing
      ...
      if (roadmapPath) {
        ...
        if (mergeResult.codeFilesChanged) {
          ...
          // (#1906) Milestone produced only .sdd/ metadata — no actual code was
          ...
          ctx.notify(
            `WARNING: Milestone ${milestoneId} merged to main but contained NO code changes — only .sdd/ metadata files. ` +
      ...
      // /sdd dispatch complete-milestone or merge manually once the underlying issue is fixed
      // (e.g. checkout to wrong branch, unresolved conflicts). (#1668)
      ctx.notify(
        `Milestone merge failed: ${msg}. Your worktree and milestone branch are preserved — retry /sdd dispatch complete-milestone or merge manually.`,
        "warning",
      );
  ...
  private _mergeBranchMode(milestoneId: string, ctx: NotifyCtx): void {
    try {
      ...
      if (mergeResult.codeFilesChanged) {
        ...
        ctx.notify(
          `WARNING: Milestone ${milestoneId} merged (branch mode) but contained NO code changes — only .sdd/ metadata. ` +
            `Review the milestone output and re-run if code is missing.`,
          "warning",
        );
      }
