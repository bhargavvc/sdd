// SDD Directory Writer — Format Functions & Directory Orchestrator
// Format functions: pure string-returning functions that serialize SDD types into the exact markdown
// format that SDD-2's parsers expect (parseRoadmap, parsePlan, parseSummary, parseRequirementCounts).
// writeSDDDirectory: orchestrator that writes a complete .sdd directory tree from a SDDProject.
...
/** Result of writeSDDDirectory — lists all files that were written. */
...
/** Pre-write statistics computed from a SDDProject without I/O. */
 ...
 */
export function formatRoadmap(milestone: SDDMilestone): string {
  const lines: string[] = [];
 ...
 */
export function formatPlan(slice: SDDSlice): string {
  const lines: string[] = [];
 ...
 */
export function formatSliceSummary(slice: SDDSlice, milestoneId: string): string {
 ...
 * Output must parse correctly through parseSummary().
 */
export function formatTaskSummary(task: SDDTask, sliceId: string, milestoneId: string): string {
 ...
 */
export function formatTaskPlan(task: SDDTask, sliceId: string, milestoneId: string): string {
  const lines: string[] = [];
 ...
 */
export function formatRequirements(requirements: SDDRequirement[]): string {
  const lines: string[] = [];
  lines.push('# Requirements');
  lines.push('');

  const groups: Record<string, SDDRequirement[]> = {
    active: [],
    validated: [],
    deferred: [],
    'out-of-scope': [],
 ...
 * Write a minimal stub that will be overwritten on first /sdd status.
 */
export function formatState(milestones: SDDMilestone[]): string {
  const lines: string[] = [];
...
/**
 * Write a complete .sdd directory tree from a SDDProject.
 ...
 */
export async function writeSDDDirectory(
  project: SDDProject,
  targetPath: string,
