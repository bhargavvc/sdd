...
function handleLostSessionLock(
  ...
  const base = lockBase();
  const lockFilePath = base ? join(sddRoot(base), "auto.lock") : "unknown";
...
export async function stopAuto(
  ...
  const loadedPreferences = loadEffectiveSDDPreferences()?.preferences;
  ...
  try {
    ...
    try {
      const pausedPath = join(sddRoot(s.originalBasePath || s.basePath), "runtime", "paused-session.json");
...
export async function pauseAuto(
  ...
  try {
    ...
    const runtimeDir = join(sddRoot(s.originalBasePath || s.basePath), "runtime");
...
function buildResolverDeps(): WorktreeResolverDeps {
  return {
    ...
    loadEffectiveSDDPreferences:
      loadEffectiveSDDPreferences as unknown as WorktreeResolverDeps["loadEffectiveSDDPreferences"],
    invalidateAllCaches,
    captureIntegrationBranch,
  };
}
...
export async function startAuto(
  ...
  if (!s.paused) {
    try {
      const pausedPath = join(sddRoot(base), "runtime", "paused-session.json");
  ...
  if (s.paused) {
    ...
    try {
      await rebuildState(s.basePath);
      syncCmuxSidebar(loadEffectiveSDDPreferences()?.preferences, await deriveState(s.basePath));
    } catch (e) {
    ...
    if (s.pausedSessionFile) {
      const activityDir = join(sddRoot(s.basePath), "activity");
    ...
    logCmuxEvent(loadEffectiveSDDPreferences()?.preferences, s.stepMode ? "Step-mode resumed." : "Auto-mode resumed.", "progress");
  ...
  logCmuxEvent(loadEffectiveSDDPreferences()?.preferences, requestedStepMode ? "Step-mode started." : "Auto-mode started.", "progress");
