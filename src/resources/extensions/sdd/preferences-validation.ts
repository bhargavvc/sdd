...
export function validatePreferences(preferences: SDDPreferences): {
  ...
  if (preferences.search_provider !== undefined) {
    ...
    if (typeof preferences.search_provider === "string" && validSearchProviders.has(preferences.search_provider)) {
      validated.search_provider = preferences.search_provider as SDDPreferences["search_provider"];
    } else {
  ...
  if (preferences.cmux !== undefined) {
    if (preferences.cmux && typeof preferences.cmux === "object") {
      ...
      const validatedCmux: NonNullable<SDDPreferences["cmux"]> = {};
  ...
  if (preferences.git && typeof preferences.git === "object") {
    ...
    if (g.commit_docs !== undefined) {
      warnings.push("git.commit_docs is deprecated — .sdd/ is managed externally and always gitignored. Remove this setting.");
    }
  ...
  if (preferences.context_selection !== undefined) {
    ...
    if (typeof preferences.context_selection === "string" && validModes.has(preferences.context_selection)) {
      validated.context_selection = preferences.context_selection as SDDPreferences["context_selection"];
    } else {
