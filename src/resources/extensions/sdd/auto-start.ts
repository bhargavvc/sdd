...
export async function bootstrapAutoSession(
  ...
  try {
    ...
    // git repo that has no .sdd (i.e. the parent project was never initialised
    // with SDD), create a fresh git repo at `base` so it gets its own identity
    ...
    s.gitService = new GitServiceImpl(
      s.basePath,
      loadEffectiveSDDPreferences()?.preferences?.git ?? {},
    );
    ...
    try {
      const runtimeUnitsDir = join(sddRoot(base), "runtime", "units");
