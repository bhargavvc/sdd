...
export function registerJournalTools(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    promptSnippet: "Query the SDD event journal with filters (flowId, unitId, rule, eventType, time range, limit)",
    promptGuidelines: [
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      try {
        ...
        process.stderr.write(`sdd-journal: sdd_journal_query tool failed: ${msg}\n`);
        return {
