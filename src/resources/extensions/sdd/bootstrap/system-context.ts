  ...
  for (const path of paths) {
    if (existsSync(path)) {
      console.warn(
        ...
        `See https://github.com/bhargavvc/SDD/issues/1492`,
      );
    }
  }
}
...
export async function buildBeforeAgentStartResult(
  ...
  const loadedPreferences = loadEffectiveSDDPreferences();
  ...
  let knowledgeBlock = "";
  const knowledgePath = resolveSddRootFile(process.cwd(), "KNOWLEDGE");
