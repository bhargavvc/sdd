 ...
 *
 * Returns true if at least one non-`.sdd/` file was committed, false otherwise.
...
export function verifyExpectedArtifact(
  ...
  // A milestone with only .sdd/ plan files and zero implementation code is
...
export function diagnoseExpectedArtifact(
  ...
  switch (unitType) {
    ...
    case "rewrite-docs":
      return "Active overrides resolved in .sdd/OVERRIDES.md + plan documents updated";
    case "reassess-roadmap":
...
/**
 * Self-heal: scan runtime records in .sdd/ and clear stale ones.
  ...
  switch (unitType) {
    case "execute-task": {
      ...
      return [
        ...
        `   3. Run \`sdd doctor\` to reconcile .sdd/ state`,
    ...
    case "research-slice": {
      ...
      return [
        ...
        `   2. Run \`sdd doctor\` to reconcile .sdd/ state`,
        `   3. Resume auto-mode`,
      ].join("\n");
    }
    case "complete-slice": {
