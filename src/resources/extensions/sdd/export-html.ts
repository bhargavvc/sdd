/**
 * SDD HTML Report Generator
 *
 * Produces a single self-contained HTML file with:
 *   - Branding header (project name, path, SDD version, generated timestamp)
...
<title>SDD Report — ${esc(opts.projectName)}${opts.milestoneId ? ` — ${esc(opts.milestoneId)}` : ''}</title>
<style>${CSS}</style>
</head>
<body>
<header>
...
(function(){
  var saved=JSON.parse(localStorage.getItem('sdd-collapsed')||'{}');
  document.querySelectorAll('section[id]').forEach(function(sec){
    ...
    btn.addEventListener('click',function(e){
      ...
      localStorage.setItem('sdd-collapsed',JSON.stringify(saved));
    });
  });
...
(function(){
  ...
  btn.textContent=localStorage.getItem('sdd-theme')==='light'?'Dark':'Light';
  if(localStorage.getItem('sdd-theme')==='light')document.documentElement.classList.add('light-theme');
