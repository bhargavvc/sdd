...
export function syncSddStateToWorktree(
  ...
  try {
    const mainResolved = realpathSync(mainSdd);
    const wtResolved = realpathSync(wtSdd);
...
function copyPlanningArtifacts(srcBase: string, wtPath: string): void {
  ...
  // Copy sdd.db if present in source
  const srcDb = join(srcSdd, "sdd.db");
  const destDb = join(dstSdd, "sdd.db");
  if (existsSync(srcDb)) {
    try {
...
export function teardownAutoWorktree(
  ...
  if (existsSync(wtDir)) {
    console.error(
      `[SDD] WARNING: Worktree directory still exists after teardown: ${wtDir}\n` +
...
export function mergeMilestoneToMain(
  ...
  // 7. Squash merge — auto-resolve .sdd/ state file conflicts (#530)
  ...
  if (!mergeResult.success) {
    ...
    if (mergeResult.conflicts.includes("__dirty_working_tree__")) {
      ...
      throw new SDDError(
        ...
        `Squash merge of ${milestoneBranch} rejected: working tree has dirty or untracked files that conflict with the merge. ` +
          `Clean the project root .sdd/ directory and retry.`,
      );
    }
    ...
    if (conflictedFiles.length > 0) {
      ...
      // SDD state files (STATE.md, auto.lock, etc.)
      ...
      if (sddConflicts.length > 0) {
        for (const sddFile of sddConflicts) {
          try {
            nativeCheckoutTheirs(originalBasePath_, [sddFile]);
            nativeAddPaths(originalBasePath_, [sddFile]);
          } catch {
            // If checkout --theirs fails, try removing the file from the merge
            // (it's a runtime file that shouldn't be committed anyway)
            nativeRmForce(originalBasePath_, [sddFile]);
          }
        }
      }

      // If there are still non-.sdd conflicts, escalate
  ...
  // 8c. Detect whether any non-.sdd/ code files were actually merged (#1906).
