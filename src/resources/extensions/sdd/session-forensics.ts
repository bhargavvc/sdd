/**
 * SDD Session Forensics — Deep analysis of pi session JSONL files
 *
 ...
 * - Stuck-retry diagnostics (reading SDD activity log copies)
 *
 ...
 * Works with both pi session JSONL and SDD activity log JSONL.
 */
 ...
 *
 * Reads the surviving pi session file (or falls back to the last SDD activity
...
export function synthesizeCrashRecovery(
  ...
  try {
    ...
    // Fallback: last SDD activity log
...
export function getDeepDiagnostic(basePath: string, worktreePath?: string): string | null {
  ...
  try {
    if (worktreePath) {
      const wtActivityDir = join(sddRoot(worktreePath), "activity");
...
export function readActiveMilestoneId(basePath: string): string | null {
  try {
    const statePath = join(sddRoot(basePath), "STATE.md");
...
function formatTraceSummary(trace: ExecutionTrace): string {
  ...
  // See: https://github.com/sdd-build/sdd-2/issues/2195
  return parts.join("\n");
}
