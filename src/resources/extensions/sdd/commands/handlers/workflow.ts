...
async function handleCustomWorkflow(
  ...
  if (sub === "run" || sub.startsWith("run ")) {
    ...
    if (!args) {
      ctx.ui.notify("Usage: /sdd workflow run <name> [param=value ...]", "warning");
      return true;
    }
    ...
    try {
      ...
      // Clean up engine state so a failed workflow run doesn't pollute the next /sdd auto
  ...
  if (sub === "validate" || sub.startsWith("validate ")) {
    ...
    if (!defName) {
      ctx.ui.notify("Usage: /sdd workflow validate <name>", "warning");
      return true;
    }
    const base = projectRoot();
    const defPath = join(base, ".sdd", "workflow-defs", `${defName}.yaml`);
  ...
  if (sub === "pause") {
    ...
    if (engineId === "dev" || engineId === null) {
      ctx.ui.notify("No custom workflow is running. Use /sdd pause for dev workflow.", "warning");
      return true;
    }
    if (!isAutoActive()) {
  ...
  if (sub === "resume") {
    const engineId = getActiveEngineId();
    if (engineId === "dev" || engineId === null) {
      ctx.ui.notify("No custom workflow to resume. Use /sdd auto for dev workflow.", "warning");
      return true;
    }
    try {
...
export async function handleWorkflowCommand(trimmed: string, ctx: ExtensionCommandContext, pi: ExtensionAPI): Promise<boolean> {
  // ── Custom workflow commands (`/sdd workflow ...`) ──
  ...
  if (trimmed === "quick" || trimmed.startsWith("quick ")) {
    if (isAutoActive()) {
      ctx.ui.notify(
        "/sdd quick cannot run while auto-mode is active.\n" +
        "Stop auto-mode first with /sdd stop, then run /sdd quick.",
        "error",
      );
      return true;
    }
  ...
  if (trimmed === "park" || trimmed.startsWith("park ")) {
    ...
    if (isParked(basePath, targetId)) {
      ctx.ui.notify(`${targetId} is already parked. Use /sdd unpark ${targetId} to reactivate.`, "info");
      return true;
    }
    const reasonParts = arg.replace(targetId, "").trim().replace(/^["']|["']$/g, "");
    const reason = reasonParts || "Parked via /sdd park";
    const success = parkMilestone(basePath, targetId, reason);
    ctx.ui.notify(
      success ? `Parked ${targetId}. Run /sdd unpark ${targetId} to reactivate.` : `Could not park ${targetId} — milestone not found.`,
  ...
  if (trimmed === "unpark" || trimmed.startsWith("unpark ")) {
    ...
    if (!targetId) {
      ...
      if (parkedEntries.length === 1) {
        ...
        ctx.ui.notify(`Parked milestones: ${parkedEntries.map((entry) => entry.id).join(", ")}. Specify which to unpark: /sdd unpark <id>`, "info");
        return true;
      }
    }
...
export function getNextMilestoneId(basePath: string): string {
  ...
  const uniqueIds = !!loadEffectiveSDDPreferences()?.preferences?.unique_milestone_ids;
