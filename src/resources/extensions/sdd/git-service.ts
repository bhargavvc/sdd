...
export interface GitPreferences {
  ...
  remote?: string;
  snapshots?: boolean;
  /** Deprecated. .sdd/ is managed externally; retained for compatibility. */
  commit_docs?: boolean;
 ...
 * Format: .sdd/milestones/<MID>/<MID>-META.json
 */
function milestoneMetaPath(basePath: string, milestoneId: string): string {
  return join(sddRoot(basePath), "milestones", milestoneId, `${milestoneId}-META.json`);
}
 ...
 */
/** Regex matching SDD quick-task branches: sdd/quick/<num>-<slug> */
...
export function writeIntegrationBranch(
  ...
  const metaFile = milestoneMetaPath(basePath, milestoneId);
  mkdirSync(join(sddRoot(basePath), "milestones", milestoneId), { recursive: true });
  ...
  // .sdd/ is managed externally (symlinked) — metadata is not committed to git.
}
...
export class GitServiceImpl {
  ...
  private smartStage(extraExclusions: readonly string[] = []): void {
    ...
    // auto.lock, etc.) — NOT all of .sdd/. If .sdd/milestones/ files were
    ...
    // discontinuity where the first half of a milestone has .sdd/ artifacts
    ...
    if (!this._runtimeFilesCleanedUp) {
      ...
      if (cleaned) {
        nativeCommit(this.basePath, "chore: untrack .sdd/ runtime files from git index", { allowEmpty: false });
      }
    ...
    //
    // Exclude only RUNTIME paths from staging — not the entire .sdd/ directory.
    // When .sdd/milestones/ files are already tracked in the index (projects
    // where .sdd/ is not gitignored, or Windows junctions that git sees as
    // real directories), they should continue to be committed. Excluding the
    // entire .sdd/ directory mid-milestone causes silent commit failure where
    // the second half of a milestone's artifacts are never committed (#1326).
    //
    // If .sdd/ IS in .gitignore (the default for external state projects),
    ...
    // Force-add .sdd/milestones/ when .sdd is a symlink (#2104).
    // When .sdd is a symlink (external state projects), ensureGitignore adds
    // `.sdd` to .gitignore. The nativeAddAllWithExclusions call above falls
    // back to plain `git add -A` (symlink pathspec rejection), which respects
    // .gitignore and silently skips new .sdd/milestones/ files.
    //
    ...
    const sddPath = join(this.basePath, ".sdd");
    const milestonesDir = join(sddPath, "milestones");
    try {
      if (
        existsSync(sddPath) &&
        lstatSync(sddPath).isSymbolicLink() &&
   ...
   * Returns paths relative to `basePath` (e.g. ".sdd/milestones/M009/SUMMARY.md").
   */
  ...
  /**
   * Stage milestone artifacts through a symlinked .sdd directory (#2104).
   *
   ...
   * @param extraExclusions Additional paths to exclude from staging (e.g. [".sdd/"] for pre-switch commits).
   */
  autoCommit(
    unitType: string,
    unitId: string,
  ...
  /** True if currently on a SDD slice branch. */
  // ─── Branch Lifecycle ──────────────────────────────────────────────────
  ...
  createSnapshot(label: string): void {
    ...
    const refPath = `refs/sdd/snapshots/${label}/${ts}`;
