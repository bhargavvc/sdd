...
A `/sdd migrate` command just wrote a `.sdd/` directory from an old `.planning` source. Your job is to audit the output and verify it meets SDD standards before the user starts working with it.
...
If the overall result is FAIL, explain what needs manual attention. If PASS WITH NOTES, explain what's imperfect but acceptable. If PASS, confirm the `.sdd` directory is ready for SDD auto-mode.
