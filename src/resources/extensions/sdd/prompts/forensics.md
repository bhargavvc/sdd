...
3. **Read the actual SDD source code** at `{{sddSourceDir}}` to confirm or deny each hypothesis. Do not guess what code does — read it.
...
**CRITICAL: The `github_issues` tool ONLY targets the current user's repository — it has no `repo` parameter. You MUST use `gh issue create --repo bhargavvc/sdd` via the `bash` tool to file on the correct repo. Do NOT use the `github_issues` tool for this.**

If yes, create using the `bash` tool:

```bash
gh issue create --repo bhargavvc/sdd \
  --title "..." \
