...
4. **Requirements analysis.** If `.sdd/REQUIREMENTS.md` exists, research against it. Identify which Active requirements are table stakes, likely omissions, overbuilt risks, or domain-standard behaviors.
...
6. If planning produced structural decisions (e.g. slice ordering rationale, technology choices, scope exclusions), append them to `.sdd/DECISIONS.md` (use the **Decisions** output template from the inlined context above if the file doesn't exist yet)
...
- If `.sdd/REQUIREMENTS.md` exists and an Active requirement has no credible path, surface that clearly. Do not silently ignore orphaned Active requirements.
