...
You are creating a SDD milestone from a provided specification document. This is a **headless** (non-interactive) flow — do NOT ask the user any questions. Work entirely from the provided specification.
...
2. Write `.sdd/PROJECT.md` (using Project template)
3. Write `.sdd/REQUIREMENTS.md` (using Requirements template)
...
6. Seed `.sdd/DECISIONS.md` (using Decisions template)
7. {{commitInstruction}}
...
2. Write `.sdd/PROJECT.md` — full vision across ALL milestones (using Project template)
3. Write `.sdd/REQUIREMENTS.md` — full capability contract (using Requirements template)
4. Seed `.sdd/DECISIONS.md` (using Decisions template)
...
- **Naming convention** — always use `sdd_milestone_generate_id` to get milestone IDs. Directories use bare IDs (e.g. `M001/` or `M001-r5jzab/`), files use ID-SUFFIX format (e.g. `M001-CONTEXT.md` or `M001-r5jzab-CONTEXT.md`). Never invent milestone IDs manually.
