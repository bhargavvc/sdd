...
- If `.sdd/REQUIREMENTS.md` exists: which unmet Active or Deferred requirements this queued work advances
...
4. **Requirement check** — If `.sdd/REQUIREMENTS.md` exists, identify whether this queued work advances unmet Active requirements, promotes Deferred work, or introduces entirely new scope that should also update the requirement contract.
...
2. Write `.sdd/milestones/<ID>/<ID>-CONTEXT.md` — use the **Context** output template below. Capture intent, scope, risks, constraints, integration points, and relevant requirements. Mark the status as "Queued — pending auto-mode execution." **If this milestone depends on other milestones, add YAML frontmatter with `depends_on`:**
   ```yaml
   ---
...
3. Update `.sdd/PROJECT.md` — add the new milestones to the Milestone Sequence. Keep existing entries exactly as they are. Only add new lines.
4. If `.sdd/REQUIREMENTS.md` exists and the queued work introduces new in-scope capabilities or promotes Deferred items, update it.
5. If discussion produced decisions relevant to existing work, append to `.sdd/DECISIONS.md`.
6. Append to `.sdd/QUEUE.md`.
7. {{commitInstruction}}

**Do NOT write roadmaps for queued milestones.**
**Do NOT update `.sdd/STATE.md`.**
