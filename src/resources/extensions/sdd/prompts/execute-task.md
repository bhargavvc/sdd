...
12. If you made an architectural, pattern, library, or observability decision during this task that downstream work should know about, append it to `.sdd/DECISIONS.md` (read the template at `~/.sdd/agent/extensions/sdd/templates/decisions.md` if the file doesn't exist yet). Not every task produces decisions — only append when a meaningful choice was made.
13. If you discover a non-obvious rule, recurring gotcha, or useful pattern during execution, append it to `.sdd/KNOWLEDGE.md`. Only add entries that would save future agents from repeating your investigation. Don't add obvious things.
14. Read the template at `~/.sdd/agent/extensions/sdd/templates/task-summary.md`
