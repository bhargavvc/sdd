...
2. {{skillActivation}}
3. **Verify code changes exist.** Run `git diff --stat HEAD $(git merge-base HEAD main) -- ':!.sdd/'` (or the equivalent for the integration branch). If no non-`.sdd/` files appear in the diff, the milestone produced only planning artifacts and no actual code. In that case, do NOT mark the milestone as passing verification — document the gap clearly in the summary and state that implementation is missing.
...
8. Update `.sdd/REQUIREMENTS.md` if any requirement status transitions were validated in step 5.
