...
5. If this slice produced evidence that a requirement changed status (Active → Validated, Active → Deferred, etc.), call `gsd_save_decision` with scope="requirement", decision="{requirement-id}", choice="{new-status}", rationale="{evidence}". Do NOT write `.sdd/REQUIREMENTS.md` directly — the engine renders it from the database.
...
8. Review task summaries for `key_decisions`. Append any significant decisions to `.sdd/DECISIONS.md` if missing.
9. Review task summaries for patterns, gotchas, or non-obvious lessons learned. If any would save future agents from repeating investigation or hitting the same issues, append them to `.sdd/KNOWLEDGE.md`. Only add entries that are genuinely useful — don't pad with obvious observations.
...
12. Update `.sdd/PROJECT.md` if it exists — refresh current state if needed.
