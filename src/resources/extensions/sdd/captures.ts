 ...
 * `.sdd/CAPTURES.md`, not the worktree's local `.sdd/`.
 */
 ...
 *
 * In worktree-isolated mode, basePath is `.sdd/worktrees/<MID>/`.
 * Captures must resolve to the *original* project root's `.sdd/CAPTURES.md`,
 * not the worktree-local `.sdd/`. This ensures all captures go to one file
 * regardless of which worktree the agent is running in.
 *
 * Detection: if basePath contains `/.sdd/worktrees/`, walk up to the
 * directory that contains `.sdd/worktrees/` — that's the project root.
 */
export function resolveCapturesPath(basePath: string): string {
  ...
  return join(sddRoot(basePath), CAPTURES_FILENAME);
}
 ...
 * Creates `.sdd/` and the file if they don't exist.
