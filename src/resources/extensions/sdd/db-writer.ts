...
export async function nextDecisionId(): Promise<string> {
  try {
    ...
    process.stderr.write(`sdd-db: nextDecisionId failed: ${(err as Error).message}\n`);
    return 'D001';
  }
}
...
export async function saveDecisionToDb(
  ...
  try {
    ...
    process.stderr.write(`sdd-db: saveDecisionToDb failed: ${(err as Error).message}\n`);
    throw err;
  }
}
...
export async function updateRequirementInDb(
  ...
  try {
    ...
    process.stderr.write(`sdd-db: updateRequirementInDb failed: ${(err as Error).message}\n`);
    throw err;
  }
}
 ...
 * The path is relative to .sdd/ (e.g. "milestones/M001/slices/S06/tasks/T01-SUMMARY.md").
 * The full file path is computed as basePath + '.sdd/' + path.
 */
export async function saveArtifactToDb(
  ...
  try {
    ...
    process.stderr.write(`sdd-db: saveArtifactToDb failed: ${(err as Error).message}\n`);
    throw err;
  }
}
