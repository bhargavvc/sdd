/**
 * SDD Doctor — Proactive Healing Layer
 *
...
import { sddRoot, resolveSddRootFile } from "./paths.js";
    ...
    if (existsSync(gitDir)) {
      ...
      if (blockers.length > 0) {
        ...
        try {
          ...
          issues.push(`Corrupt git state: ${blockers.join(", ")}. Run /sdd doctor fix.`);
        }
      }
    }
  } catch {
    ...
    if (nativeIsRepo(basePath)) {
      ...
      if (state.activeMilestone) {
        ...
        if (resolution.status === "fallback" && resolution.effectiveBranch) {
          ...
          issues.push(
            `${resolution.reason} Restore the branch or update the integration branch before dispatching. Run /sdd doctor for details.`,
          );
        }
      }
    }
