...
function activityDir(basePath: string): string {
  return join(sddRoot(basePath), "activity");
}

function debugDir(basePath: string): string {
  return join(sddRoot(basePath), "debug");
}
...
async function handleLogsList(basePath: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  if (activities.length > 0) {
    lines.push("Activity Logs (.sdd/activity/):");
  ...
  if (debugLogs.length > 0) {
    lines.push("");
    lines.push("Debug Logs (.sdd/debug/):");
  ...
  // Metrics summary
  const metricsPath = join(sddRoot(basePath), "metrics.json");
...
async function handleLogsDebug(basePath: string, ctx: ExtensionCommandContext, idx?: number): Promise<void> {
  ...
  if (idx === undefined) {
    // List debug logs
    const lines: string[] = ["Debug Logs (.sdd/debug/):", ""];
    for (let i = 0; i < debugLogs.length; i++) {
      const d = debugLogs[i];
