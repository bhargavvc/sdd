...
function loadProvider(): void {
  ...
  const versionHint = nodeMajor < 22
    ? ` SDD requires Node >= 22.0.0 (current: v${process.versions.node}). Upgrade Node to fix this.`
    : "";
  process.stderr.write(
    `sdd-db: No SQLite provider available (tried node:sqlite, better-sqlite3).${versionHint}\n`,
  );
}
