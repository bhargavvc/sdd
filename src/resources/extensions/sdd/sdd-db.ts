// SDD Database Abstraction Layer
...
function loadProvider(): void {
  ...
  process.stderr.write(
    "sdd-db: No SQLite provider available (tried node:sqlite, better-sqlite3)\n",
  );
}
...
export function transaction<T>(fn: () => T): T {
  if (!currentDb)
    throw new SDDError(SDD_STALE_STATE, "sdd-db: No database open");
...
export function copyWorktreeDb(srcDbPath: string, destDbPath: string): boolean {
  try {
    ...
    process.stderr.write(
      `sdd-db: failed to copy DB to worktree: ${(err as Error).message}\n`,
    );
    return false;
  }
}
...
export function reconcileWorktreeDb(
  ...
  if (worktreeDbPath.includes("'")) {
    process.stderr.write(
      `sdd-db: worktree DB reconciliation failed: path contains unsafe characters\n`,
    );
    return zero;
  }
  if (!currentDb) {
    ...
    if (!opened) {
      process.stderr.write(
        `sdd-db: worktree DB reconciliation failed: cannot open main DB\n`,
      );
      return zero;
    }
  }
  ...
  try {
    ...
    process.stderr.write(
      `sdd-db: worktree DB reconciliation failed: ${(err as Error).message}\n`,
    );
