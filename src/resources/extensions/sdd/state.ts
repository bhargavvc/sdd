...
// of re-reading the entire .sdd/ tree from disk.
...
/**
 * Reconstruct SDD state from files on disk.
 ...
 * every .md file under .sdd/, populating an in-memory cache that replaces all
 ...
 */
export async function deriveState(basePath: string): Promise<SDDState> {
...
async function _deriveStateImpl(basePath: string): Promise<SDDState> {
  const milestoneIds = findMilestoneIds(basePath);
  ...
  // When the native Rust parser is available, read every .md file under .sdd/
