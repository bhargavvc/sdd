// SDD Extension — Session/Milestone Export
...
export function writeExportFile(
  ...
  const exportDir = sddRoot(basePath);
  ...
  if (format === "json") {
    ...
    const md = [
      `# SDD Session Report — ${projectName}`,
      ``,
  ...
  if (args.includes("--html")) {
    ...
    try {
      ...
      if (generateAll) {
        ...
        const indexPath = join(sddRoot(basePath), "reports", "index.html");
        ctx.ui.notify(
        ...
        ctx.ui.notify(
          `HTML report saved: .sdd/reports/${bn(outPath)}\nOpening in browser...`,
          "success",
        );
