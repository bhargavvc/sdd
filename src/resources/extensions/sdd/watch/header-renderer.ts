// SDD Watch — Header renderer: ASCII logo, session info, MCP status, remote questions
...
import { visibleWidth, truncateToWidth } from "@sdd/pi-tui";
import { loadEffectiveSDDPreferences } from "../preferences.js";

// ─── Constants ────────────────────────────────────────────────────────────────

/**
 * SDD ASCII logo — inlined here because the canonical src/logo.ts is outside
 * the resources rootDir and cannot be imported directly.
 */
const SDD_LOGO: readonly string[] = [
  '   ██████╗ ███████╗██████╗ ',
  '  ██╔════╝ ██╔════╝██╔══██╗',
  '  ██║  ███╗███████╗██║  ██║',
  '  ██║   ██║╚════██║██║  ██║',
...
/**
 * Read the configured execution model from SDD preferences.
 ...
 *
 * Layout: SDD ASCII logo on the left, info panel on the right separated by │.
...
export function renderHeaderLines(data: HeaderData, width: number): string[] {
  ...
  const infoLines: string[] = [
    `\x1b[1mSpec-Driven Development\x1b[0m`,
    "",
...
function renderStackedHeader(data: HeaderData, width: number): string[] {
  ...
  // Title
  lines.push(`\x1b[1mSpec-Driven Development\x1b[0m`);
  lines.push("");
