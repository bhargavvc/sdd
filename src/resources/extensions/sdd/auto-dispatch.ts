...
export const DISPATCH_RULES: DispatchRule[] = [
  ...
  },
  {
    ...
    match: async ({ state, mid, midTitle, basePath }) => {
      ...
      // artifacts (non-.sdd/ files). A milestone with only plan files and
      // zero implementation code should not be marked complete.
      if (!hasImplementationArtifacts(basePath)) {
        return {
          action: "stop",
          reason: `Cannot complete milestone ${mid}: no implementation files found outside .sdd/. The milestone has only plan files — actual code changes are required.`,
          level: "error",
        };
      }
