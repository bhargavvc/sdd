...
import { autoWorktreeBranch, runWorktreePostCreateHook, syncSddStateToWorktree } from "./auto-worktree.js";
...
/**
 * Persist the current orchestrator state to .sdd/orchestrator.json.
...
/**
 * Restore orchestrator state from .sdd/orchestrator.json.
...
function restoreRuntimeState(basePath: string): boolean {
  ...
  // still running and writing heartbeats under .sdd/parallel/.
...
export async function prepareParallelStart(
  basePath: string,
  _prefs: SDDPreferences | undefined,
...
function createMilestoneWorktree(basePath: string, milestoneId: string): string {
  ...
  // Copy .sdd/ planning artifacts (milestones, CONTEXT, ROADMAP, etc.) from the
  ...
  syncSddStateToWorktree(basePath, info.path);
 ...
 * The worker runs `sdd headless --json auto` in the milestone's worktree
 * with SDD_MILESTONE_LOCK set to isolate state derivation.
 *
 * IMPORTANT: We use `headless --json auto` instead of `--print "/sdd auto"`.
...
function resolveSddBin(): string | null {
  ...
  // This file is at dist/resources/extensions/sdd/parallel-orchestrator.js
