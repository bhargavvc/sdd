 ...
 * Returns a partial SDDPreferences that is used as the base layer --
 * explicit user preferences always override these defaults.
 */
export function resolveProfileDefaults(profile: TokenProfile): Partial<SDDPreferences> {
  switch (profile) {
    case "budget":
      return {
        models: {
 ...
 */
export function resolveSearchProviderFromPreferences(): SDDPreferences["search_provider"] | undefined {
  const prefs = loadEffectiveSDDPreferences();
