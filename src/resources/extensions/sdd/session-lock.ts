 ...
 *
 * The lock file (.sdd/auto.lock) contains JSON metadata (PID, start time,
...
function ensureExitHandler(_sddDir: string): void {
  ...
  process.once("exit", () => {
    ...
    // Lock files accumulate across main project .sdd/, worktree .sdd/,
      ...
      onCompromised: () => {
        ...
        if (elapsed < 1_800_000) {
          process.stderr.write(
            `[sdd] Lock heartbeat mismatch after ${Math.round(elapsed / 1000)}s — event loop stall, continuing.\n`,
          );
...
export function releaseSessionLock(basePath: string): void {
  ...
  // main project .sdd/, worktree .sdd/, and projects registry paths.
  for (const dir of _lockDirRegistry) {
    try {
      const lockFile = join(dir, LOCK_FILE);
      if (existsSync(lockFile)) unlinkSync(lockFile);
