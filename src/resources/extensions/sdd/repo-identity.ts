 ...
 *   3. There is no `.sdd` directory at the git root (the parent project
 *      has not been initialised with SDD)
 *
 ...
 *
 * When the git root already has `.sdd`, the directory is a legitimate
 * subdirectory of an existing SDD project — `cd src/ && /sdd` should
...
export function isInheritedRepo(basePath: string): boolean {
  try {
    ...
    if (existsSync(join(root, ".sdd"))) return false;

    // Also walk up from basePath to the git root checking for .sdd
    let dir = normalizedBase;
    while (dir !== normalizedRoot && dir !== dirname(dir)) {
      if (existsSync(join(dir, ".sdd"))) return false;
      dir = dirname(dir);
    }
 ...
 * 2. If `<project>/.sdd` doesn't exist → create symlink
 * 3. If `<project>/.sdd` is already the correct symlink → no-op
 * 4. If `<project>/.sdd` is a real directory → return as-is (migration handles later)
 *
