...
import { runSDDDoctor, rebuildState, summarizeDoctorIssues } from "./doctor.js";
...
export interface PostUnitContext {
  ...
  updateProgressWidget: (ctx: ExtensionContext, unitType: string, unitId: string, state: import("./types.js").SDDState) => void;
}
...
export async function postUnitPreVerification(pctx: PostUnitContext, opts?: PreVerificationOpts): Promise<"dispatched" | "continue" | "retry"> {
  ...
  if (s.currentUnit) {
    ...
    if (!opts?.skipDoctor) try {
      ...
      const report = await runSDDDoctor(s.basePath, { fix: true, scope: doctorScope, fixLevel: effectiveFixLevel });
...
export async function postUnitPostVerification(pctx: PostUnitContext): Promise<"continue" | "step-wizard" | "stopped"> {
  ...
  if (isDbAvailable()) {
    try {
      ...
      process.stderr.write(`sdd-db: re-import failed: ${(err as Error).message}\n`);
    }
  }
