...
export function resolveHookArtifactPath(basePath: string, unitId: string, artifactName: string): string {
  ...
  if (parts.length === 3) {
    ...
    return join(basePath, ".sdd", "milestones", mid, "slices", sid, "tasks", `${tid}-${artifactName}`);
  }
  if (parts.length === 2) {
    const [mid, sid] = parts;
    return join(basePath, ".sdd", "milestones", mid, "slices", sid, artifactName);
  }
  return join(basePath, ".sdd", "milestones", parts[0], artifactName);
}
