// SDD Extension — Unified Rule Registry
//
...
export function resolveHookArtifactPath(basePath: string, unitId: string, artifactName: string): string {
  ...
  if (task !== undefined && slice !== undefined) {
    return join(basePath, ".sdd", "milestones", milestone, "slices", slice, "tasks", `${task}-${artifactName}`);
  }
  if (slice !== undefined) {
    return join(basePath, ".sdd", "milestones", milestone, "slices", slice, artifactName);
  }
  return join(basePath, ".sdd", "milestones", milestone, artifactName);
}
...
export class RuleRegistry {
  ...
  private _hookStatePath(basePath: string): string {
    return join(basePath, ".sdd", HOOK_STATE_FILE);
  }
  ...
  formatHookStatus(): string {
    ...
    if (entries.length === 0) {
      return "No hooks configured. Add post_unit_hooks or pre_dispatch_hooks to .sdd/PREFERENCES.md";
    }
