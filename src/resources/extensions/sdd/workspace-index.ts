...
export interface IndexWorkspaceOptions {
   ...
   * for explicit doctor/audit flows. The /sdd status dashboard and scope pickers
...
export async function getSuggestedNextCommands(basePath: string): Promise<string[]> {
  // Run validation here since we surface a /sdd doctor audit hint when issues exist.
  ...
  if (index.validationIssues.length > 0 && scope) commands.add(`/sdd doctor audit ${scope}`);
