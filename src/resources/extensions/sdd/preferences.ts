...
export function getLegacyGlobalSDDPreferencesPath(): string {
  return legacyGlobalPreferencesPath();
}

export function getProjectSDDPreferencesPath(): string {
  return projectPreferencesPath();
}

// ─── Loading ────────────────────────────────────────────────────────────────

export function loadGlobalSDDPreferences(): LoadedSDDPreferences | null {
  return loadPreferencesFile(globalPreferencesPath(), "global")
    ?? loadPreferencesFile(globalPreferencesPathLegacy(), "global")
    ?? loadPreferencesFile(legacyGlobalPreferencesPath(), "global");
}

export function loadProjectSDDPreferences(): LoadedSDDPreferences | null {
  return loadPreferencesFile(projectPreferencesPath(), "project")
    ?? loadPreferencesFile(projectPreferencesPathLegacy(), "project");
}

export function loadEffectiveSDDPreferences(): LoadedSDDPreferences | null {
  const globalPreferences = loadGlobalSDDPreferences();
  const projectPreferences = loadProjectSDDPreferences();

  if (!globalPreferences && !projectPreferences) return null;

  let result: LoadedSDDPreferences;
  ...
  if (profile) {
    ...
    result = {
      ...result,
      preferences: mergePreferences(profileDefaults as SDDPreferences, result.preferences),
    };
  }
...
function loadPreferencesFile(path: string, scope: "global" | "project"): LoadedSDDPreferences | null {
...
export function parsePreferencesMarkdown(content: string): SDDPreferences | null {
...
function parseFrontmatterBlock(frontmatter: string): SDDPreferences {
  try {
    const parsed = parseYaml(frontmatter);
    if (typeof parsed !== 'object' || parsed === null) {
      return {} as SDDPreferences;
    }
    return parsed as SDDPreferences;
  } catch (e) {
...
/**
 * Parse heading+list format into a nested object, then cast to SDDPreferences.
...
function parseHeadingListFormat(content: string): SDDPreferences {
  ...
  return typed as SDDPreferences;
}
 ...
 */
export function applyModeDefaults(mode: WorkflowMode, prefs: SDDPreferences): SDDPreferences {
  const defaults = MODE_DEFAULTS[mode];
  if (!defaults) return prefs;
  return mergePreferences(defaults, prefs);
}

function mergePreferences(base: SDDPreferences, override: SDDPreferences): SDDPreferences {
  return {
...
export function resolveParallelConfig(prefs: SDDPreferences | undefined): import("./types.js").ParallelConfig {
  return {
