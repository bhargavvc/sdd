 ...
 *
 * Each run lives under `.sdd/workflow-runs/<name>/<timestamp>/` and contains:
 ...
 *
 * 1. Loads the definition from `<basePath>/.sdd/workflow-defs/<defName>.yaml`
 * 2. Applies parameter substitution if overrides are provided
 * 3. Creates `<basePath>/.sdd/workflow-runs/<defName>/<timestamp>/`
...
export function createRun(
  ...
  const defsDir = join(basePath, ".sdd", DEFS_DIR);
  ...
  const runDir = join(basePath, ".sdd", RUNS_DIR, defName, timestamp);
 ...
 *
 * Scans `<basePath>/.sdd/workflow-runs/` for run directories. Each run's
...
export function listRuns(basePath: string, defName?: string): RunMetadata[] {
  const runsRoot = join(basePath, ".sdd", RUNS_DIR);
