...
/**
 * Returns true when engine tables are empty AND a .sdd/milestones/ directory
...
/**
 * Migrate legacy markdown-only .sdd/ projects to engine DB state.
 * Reads .sdd/milestones/<ID>/ directories and parses ROADMAP.md, *-PLAN.md
...
export function migrateFromMarkdown(basePath: string): void {
  ...
  if (milestoneDirs.length === 0) {
    process.stderr.write("workflow-migration: no milestone directories found in .sdd/milestones/\n");
    return;
  }
