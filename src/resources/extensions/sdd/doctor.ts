...
/**
 * Characters that are used as delimiters in SDD state management documents
 ...
 *   and git branch names (sdd/M001/S01). A slash in a title can break path resolution.
 */
 ...
 * with SDD's state document delimiter conventions.
...
export function validateTitle(title: string): string | null {
  if (TITLE_DELIMITER_RE.test(title)) {
    ...
    return `title contains ${found.join(" and ")}, which conflict with SDD state document delimiters`;
  }
  return null;
}

function validatePreferenceShape(preferences: SDDPreferences): string[] {
  const issues: string[] = [];
...
async function ensureSliceUatStub(basePath: string, milestoneId: string, sliceId: string, fixesApplied: string[]): Promise<void> {
  ...
  const content = [
    ...
    "2. **Expected:** SDD doctor reports no remaining completion drift for this slice.",
    "",
    "## Failure Signals",
...
export async function runSDDDoctor(basePath: string, options?: { fix?: boolean; dryRun?: boolean; scope?: string; fixLevel?: "task" | "all"; isolationMode?: "none" | "worktree" | "branch"; includeBuild?: boolean; includeTests?: boolean }): Promise<DoctorReport> {
  const issues: DoctorIssue[] = [];
  ...
  if (prefs) {
    ...
    for (const issue of prefIssues) {
      issues.push({
        ...
        scope: "project",
        unitId: "project",
        message: `SDD preferences invalid: ${issue}`,
        file: prefs.path,
        fixable: false,
      });
    }
  ...
  if (!existsSync(milestonesPath)) {
    const report: DoctorReport = { ok: issues.every(i => i.severity !== "error"), basePath, issues, fixesApplied, timing: { git: gitMs, runtime: runtimeMs, environment: envMs, sddState: 0 } };
  ...
  const report: DoctorReport = {
    ...
    issues,
    fixesApplied,
    timing: { git: gitMs, runtime: runtimeMs, environment: envMs, sddState: Math.max(0, Date.now() - t0env - envMs) },
  };
  await appendDoctorHistory(basePath, report);
  return report;
}
