...
/**
 * Patterns matching authoritative .sdd/ state files that agents must NOT write directly.
 *
 * Only STATE.md is blocked — it is purely engine-rendered from DB state.
 * All other .sdd/ files are agent-authored content that agents create and
...
/**
 * Tests whether the given file path matches a blocked authoritative .sdd/ state file.
...
/**
 * Error message returned when an agent attempts to directly write an authoritative .sdd/ state file.
