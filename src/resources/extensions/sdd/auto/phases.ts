    ...
    if (prefs?.git?.auto_pr) {
      try {
        ...
        const prUrl = createDraftPR(
          s.basePath,
          s.currentMilestoneId!,
          `[SDD] ${s.currentMilestoneId} complete`,
          `Milestone ${s.currentMilestoneId} completed by SDD auto-mode.\n\nSee .sdd/${s.currentMilestoneId}/ for details.`,
        );
        if (prUrl) {
...
export async function runUnitPhase(
  ...
  try {
    ...
    process.stderr.write(
      `[sdd] prompt reorder failed (non-fatal): ${msg}\n`,
    );
  }
