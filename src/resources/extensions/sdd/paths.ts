...
function cachedReaddirWithTypes(dirPath: string): Dirent[] {
  ...
  // Try native tree cache for paths under .sdd/
  if (nativeTreeBase) {
...
// ─── Relative Path Builders (for prompts — .sdd/milestones/...) ────────────

/**
 * Build relative .sdd/ path to a milestone directory.
...
export function relMilestonePath(basePath: string, milestoneId: string): string {
  const dir = resolveDir(milestonesDir(basePath), milestoneId);
  if (dir) return `.sdd/milestones/${dir}`;
  return `.sdd/milestones/${milestoneId}`;
}

/**
 * Build relative .sdd/ path to a milestone file.
 */
...
/**
 * Build relative .sdd/ path to a slice directory.
 */
...
/**
 * Build relative .sdd/ path to a slice file.
 */
...
/**
 * Build relative .sdd/ path to a task file.
 */
