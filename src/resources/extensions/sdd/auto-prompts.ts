...
function resolvePreferenceSkillNames(refs: string[], base: string): string[] {
  ...
  const prefs: SDDPreferences = { always_use_skills: refs };
...
export function buildSkillActivationBlock(params: {
  ...
  preferences?: SDDPreferences;
}): string {
  const prefs = params.preferences ?? loadEffectiveSDDPreferences()?.preferences;
...
export async function checkNeedsReassessment(
  base: string, mid: string, state: SDDState,
...
export async function checkNeedsRunUat(
  base: string, mid: string, state: SDDState, prefs: SDDPreferences | undefined,
...
export async function buildDiscussMilestonePrompt(mid: string, midTitle: string, base: string): Promise<string> {
  ...
  const basePrompt = loadPrompt("guided-discuss-milestone", {
    ...
    commitInstruction: "Do not commit planning artifacts — .sdd/ is managed externally.",
  });
...
export async function buildPlanSlicePrompt(
  ...
  const commitInstruction = commitDocsEnabled
    ? `Commit the plan files only: \`git add ${relSlicePath(base, mid, sid)}/ .sdd/DECISIONS.md .gitignore && git commit -m "docs(${sid}): add slice plan"\`. Do not stage .sdd/STATE.md or other runtime files — the system manages those.`
...
export async function buildReassessRoadmapPrompt(
  ...
  const reassessPrefs = loadEffectiveSDDPreferences();
  ...
  const reassessCommitInstruction = reassessCommitDocsEnabled
    ? `Commit: \`docs(${mid}): reassess roadmap after ${completedSliceId}\`. Stage only the .sdd/milestones/ files you changed — do not stage .sdd/STATE.md or other runtime files.`
    : "Do not commit — planning docs are not tracked in git for this project.";
