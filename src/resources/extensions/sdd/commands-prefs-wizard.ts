...
import {
  ...
  getLegacyGlobalSDDPreferencesPath,
  getProjectSDDPreferencesPath,
  loadGlobalSDDPreferences,
  loadProjectSDDPreferences,
...
export async function handlePrefs(args: string, ctx: ExtensionCommandContext): Promise<void> {
  ...
  if (trimmed === "project" || trimmed === "wizard project" || trimmed === "setup project") {
    await ensurePreferencesFile(getProjectSDDPreferencesPath(), ctx, "project");
  ...
  if (trimmed === "status") {
    const globalPrefs = loadGlobalSDDPreferences();
    const projectPrefs = loadProjectSDDPreferences();
    const canonicalGlobal = getGlobalSDDPreferencesPath();
    const legacyGlobal = getLegacyGlobalSDDPreferencesPath();
