// SDD State Machine — Comprehensive Phase-by-Phase Walkthrough Tests
...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), "sdd-walkthrough-"));
...
function writeTaskSummary(base: string, mid: string, sid: string, tid: string): void {
  const tasksDir = join(base, ".sdd", "milestones", mid, "slices", sid, "tasks");
  mkdirSync(tasksDir, { recursive: true });
...
describe("state-machine-full-walkthrough", () => {
  ...
  describe("Phase 5: planning", () => {
    ...
    test("PLAN exists but zero tasks → planning", async () => {
      ...
      const dir = join(base, ".sdd", "milestones", "M001", "slices", "S01");
      mkdirSync(dir, { recursive: true });
