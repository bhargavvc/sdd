...
export function resolve(specifier, context, nextResolve) {
  ...
  if (specifier.includes('@sdd/')) {
    tsSpecifier = specifier.replace('@sdd/', PACKAGES_ROOT).replace('/dist/', '/src/');
    if (tsSpecifier.includes('/packages/pi-ai') && !tsSpecifier.endsWith('.ts')) {
        ...
        // Fallback for other sdd packages like pi-coding-agent, pi-tui, pi-agent-core
