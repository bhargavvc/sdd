...
test("formatNotificationTitle returns 'SDD' when no project name is given", () => {
  assert.equal(formatNotificationTitle(), "SDD");
  assert.equal(formatNotificationTitle(undefined), "SDD");
  assert.equal(formatNotificationTitle(""), "SDD");
});

test("formatNotificationTitle includes project name when provided", () => {
  assert.equal(formatNotificationTitle("my-app"), "SDD — my-app");
});

test("formatNotificationTitle trims whitespace from project name", () => {
  assert.equal(formatNotificationTitle("  spaced  "), "SDD — spaced");
});
...
test("buildDesktopNotificationCommand includes project name in title on linux", () => {
  ...
  assert.ok(command);
  assert.equal(command.args[2], "SDD — my-project");
...
test("buildDesktopNotificationCommand includes project name in title on macOS", () => {
  ...
  if (command.file.includes("terminal-notifier")) {
    ...
    assert.equal(command.args[titleIdx + 1], "SDD — my-project");
  } else {
    assert.match(command.args[1], /SDD — my-project/);
  }
});
