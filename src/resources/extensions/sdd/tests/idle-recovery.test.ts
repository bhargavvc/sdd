...

{
  ...
  try {
    ...
    assertTrue(result!.includes("sdd doctor"), "steps include sdd doctor command");
...

{
  ...
  const base = mkdtempSync(join(tmpdir(), "sdd-loop-recovery-test-"));
  try {
    mkdirSync(join(base, ".sdd", "milestones", "M002", "slices", "S03", "tasks"), { recursive: true });
    const planPath = join(base, ".sdd", "milestones", "M002", "slices", "S03", "S03-PLAN.md");
    writeFileSync(planPath, [
    ...
    const summaryPath = join(base, ".sdd", "milestones", "M002", "slices", "S03", "tasks", "T01-SUMMARY.md");
