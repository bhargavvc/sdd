 ...
 * target source code (non-.sdd/ paths) during queue mode.
 *
 ...
 *   (d) write/edit to .sdd/ path → pass (planning artifacts)
 *   (e) write/edit to source path → block
 *   (f) bash command → block (could execute work)
 *   (g) registered SDD tools (sdd_milestone_generate_id, sdd_summary_save) → pass
 */
...
test('queue-guard: allows discussion and planning tools during queue mode', () => {
  ...
  const r2 = shouldBlockQueueExecution('sdd_milestone_generate_id', '', true);
  assert.strictEqual(r2.block, false, 'sdd_milestone_generate_id should pass');

  const r3 = shouldBlockQueueExecution('sdd_summary_save', '', true);
  assert.strictEqual(r3.block, false, 'sdd_summary_save should pass');
});

// ─── Scenario 4: Write to .sdd/ paths passes (planning artifacts) ──

test('queue-guard: allows writes to .sdd/ paths during queue mode', () => {
  const r1 = shouldBlockQueueExecution('write', '.sdd/milestones/M001/M001-CONTEXT.md', true);
  assert.strictEqual(r1.block, false, 'write to .sdd/ should pass');

  const r2 = shouldBlockQueueExecution('write', '/project/.sdd/PROJECT.md', true);
  assert.strictEqual(r2.block, false, 'write to .sdd/PROJECT.md should pass');

  const r3 = shouldBlockQueueExecution('edit', '.sdd/QUEUE.md', true);
  assert.strictEqual(r3.block, false, 'edit to .sdd/QUEUE.md should pass');

  const r4 = shouldBlockQueueExecution('write', '.sdd/REQUIREMENTS.md', true);
  assert.strictEqual(r4.block, false, 'write to .sdd/REQUIREMENTS.md should pass');

  const r5 = shouldBlockQueueExecution('write', '.sdd/DECISIONS.md', true);
  assert.strictEqual(r5.block, false, 'write to .sdd/DECISIONS.md should pass');
});
...
// ─── Scenario 8: mkdir for .sdd/ milestone directories passes ──

test('queue-guard: allows mkdir for .sdd/ milestone directories', () => {
  const r1 = shouldBlockQueueExecution('bash', 'mkdir -p .sdd/milestones/M010/slices', true);
  assert.strictEqual(r1.block, false, 'mkdir -p .sdd/ should pass');
});
