...
test("dispatch guard skips cross-milestone check when SDD_MILESTONE_LOCK is set (#2797)", (t) => {
  const repo = setupRepo();
  t.after(() => {
  ...
  mkdirSync(join(repo, ".sdd", "milestones", "M010"), { recursive: true });
  mkdirSync(join(repo, ".sdd", "milestones", "M011"), { recursive: true });
  mkdirSync(join(repo, ".sdd", "milestones", "M012"), { recursive: true });
  ...
  writeFileSync(join(repo, ".sdd", "milestones", "M010", "M010-ROADMAP.md"), "# M010\n");
  writeFileSync(join(repo, ".sdd", "milestones", "M011", "M011-ROADMAP.md"), "# M011\n");
  writeFileSync(join(repo, ".sdd", "milestones", "M012", "M012-ROADMAP.md"), "# M012\n");
  ...
  process.env.SDD_MILESTONE_LOCK = "M012";
  assert.equal(
    getPriorSliceCompletionBlocker(repo, "main", "execute-task", "M012/S01/T01"),
    null,
  );
