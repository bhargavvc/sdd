...
describe("auto-worktree lifecycle", () => {
  ...
  test("create → detect → teardown", () => {
    ...
    // Create .sdd/milestones/M003 with a dummy file (simulates planning artifacts)
    const msDir = join(tempDir, ".sdd", "milestones", "M003");
    ...
    assert.ok(
      existsSync(join(wtPath, ".sdd", "milestones", "M003", "CONTEXT.md")),
  ...
  test("#1526: getMainBranch returns milestone/<MID> in auto-worktree", async () => {
    tempDir = createTempRepo();
    const msDir = join(tempDir, ".sdd", "milestones", "M005");
    mkdirSync(msDir, { recursive: true });
  ...
  test("#1713: stale worktree directory without .git file", async () => {
    tempDir = createTempRepo();
    const msDir = join(tempDir, ".sdd", "milestones", "M010");
    mkdirSync(msDir, { recursive: true });
  ...
  test("#778: reconcile plan checkboxes on re-attach", async () => {
    ...
    const planRelPath = join(".sdd", "milestones", "M004", "slices", "S01", "S01-PLAN.md");
    const planDir = join(tempDir, ".sdd", "milestones", "M004", "slices", "S01");
  ...
  test("#2791: mcp.json copied into worktree via copyPlanningArtifacts", () => {
    ...
    // Create mcp.json in .sdd/ AFTER the commit (untracked, like real usage).
    // copyPlanningArtifacts should copy it into the worktree's .sdd/.
    writeFileSync(
      join(tempDir, ".sdd", "mcp.json"),
    ...
    try {
      assert.ok(
        existsSync(join(wtPath, ".sdd", "mcp.json")),
        "mcp.json should be copied into worktree .sdd/ on creation",
      );
    } finally {
      teardownAutoWorktree(tempDir, "M003");
    }
  ...
  test("#2791: mcp.json synced via syncSddStateToWorktree (ROOT_STATE_FILES)", () => {
    tempDir = createTempRepo();
    const msDir = join(tempDir, ".sdd", "milestones", "M003");
    mkdirSync(msDir, { recursive: true });
    writeFileSync(join(msDir, "CONTEXT.md"), "# M003 Context\n");
    ...
    try {
      // Now add mcp.json to the main .sdd/ after worktree was created
      writeFileSync(
        join(tempDir, ".sdd", "mcp.json"),
        JSON.stringify({ servers: { test: { command: "echo" } } }),
      );
      ...
      const { synced } = syncSddStateToWorktree(tempDir, wtPath);
