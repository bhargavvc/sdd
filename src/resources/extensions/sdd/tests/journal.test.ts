...
function makeTmpBase(): string {
  const base = join(tmpdir(), `sdd-journal-test-${randomUUID()}`);
...
describe("emitJournalEvent", () => {
  ...
  test("creates journal directory and JSONL file", () => {
    ...
    const filePath = join(base, ".sdd", "journal", "2025-03-21.jsonl");
  ...
  test("silently catches read-only directory errors", () => {
    const journalDir = join(base, ".sdd", "journal");
...
describe("emitJournalEvent — auto-creates parent directory", () => {
  let base: string;
  beforeEach(() => {
    base = join(tmpdir(), `sdd-journal-test-${randomUUID()}`);
    // Don't create .sdd/ — emitJournalEvent should handle it via mkdirSync recursive
  });
  afterEach(() => { cleanup(base); });
