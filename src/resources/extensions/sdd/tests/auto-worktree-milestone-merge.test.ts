...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Note: loadEffectiveSDDPreferences uses a module-level const for project
    ...
    // ─── Test 5: Auto-resolve .sdd/ state file conflicts (#530) ───────
    console.log("\n=== auto-resolve .sdd/ state file conflicts ===");
    {
      const repo = freshRepo();
      ...
      // Modify .sdd/STATE.md on the milestone branch (simulates auto-mode state updates)
      ...
      // Now modify .sdd/STATE.md on main too (simulates divergence)
      ...
      // Merge should succeed despite .sdd/STATE.md conflict — auto-resolved
      let threw = false;
      try {
        const result = mergeMilestoneToMain(repo, "M050", roadmap);
        assertTrue(result.commitMessage.includes("feat(M050)"), "merge commit created despite .sdd conflict");
      } catch (err) {
        threw = true;
      }
      assertTrue(!threw, "auto-resolves .sdd/ state file conflicts without throwing");
    ...
    console.log("\n=== #1738 bug 3: synced .sdd/ dirs cleaned before merge ===");
    {
      const repo = freshRepo();
      ...
      // Simulate syncStateToProjectRoot: create untracked .sdd/ milestone files
      ...
      assertTrue(!threw, "#1738 merge does not fail on synced .sdd/ files");
    ...
    // ─── Test 18: #1906 — codeFilesChanged false when only .sdd/ metadata merged ──
    console.log("\n=== #1906: codeFilesChanged=false when only .sdd/ metadata merged ===");
    {
      const repo = freshRepo();
      const wtPath = createAutoWorktree(repo, "M180");

      // Only add .sdd/ metadata files — no actual code
      ...
      assertEq(
        ...
        false,
        "#1906: codeFilesChanged must be false when only .sdd/ files were merged",
      );
    }
