...
async function main(): Promise<void> {
  ...
  console.log('\n=== Full pipeline: parse → transform → preview → write → deriveState ===');
  {
    ...
    try {
      ...
      assertTrue(existsSync(join(sdd, 'PROJECT.md')), 'pipeline: PROJECT.md written');
      assertTrue(existsSync(join(sdd, 'STATE.md')), 'pipeline: STATE.md written');
      assertTrue(existsSync(join(sdd, 'REQUIREMENTS.md')), 'pipeline: REQUIREMENTS.md written');
  ...
  console.log('\n=== .sdd/ exists detection ===');
  {
    const base = mkdtempSync(join(tmpdir(), 'sdd-cmd-exists-'));
    try {
      // No .sdd/ yet
      assertTrue(!existsSync(join(base, '.sdd')), 'exists-detection: .sdd absent initially');

      // Create .sdd/
      mkdirSync(join(base, '.sdd'), { recursive: true });
      assertTrue(existsSync(join(base, '.sdd')), 'exists-detection: .sdd detected after creation');
    } finally {
      rmSync(base, { recursive: true, force: true });
    }
  }
