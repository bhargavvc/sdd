 ...
 *
 * Tests the full lifecycle of SDD operations inside a worktree:
 * - Branch namespacing (sdd/<wt>/<M>/<S> instead of sdd/<M>/<S>)
...
const base = mkdtempSync(join(tmpdir(), "sdd-wt-integration-"));
...
writeFileSync(
  join(base, ".sdd", "milestones", "M001", "slices", "S02", "S02-PLAN.md"),
  "# S02: Second\n\n**Goal:** Demo\n**Demo:** Demo\n\n## Must-Haves\n- done\n\n## Tasks\n- [ ] **T01: Implement** `est:10m`\n  do it\n",
  "utf-8",
);
run("git add .", base);
...
describe('worktree-integration', async () => {
  ...
  assert.deepStrictEqual(getSliceBranchName("M001", "S01", "alpha"), "sdd/alpha/M001/S01", "explicit worktree param");
  assert.deepStrictEqual(getSliceBranchName("M001", "S01"), "sdd/M001/S01", "no worktree param = plain branch");
  ...
  assert.deepStrictEqual(getCurrentBranch(wt.path), "sdd/alpha/M001/S01", "worktree-namespaced slice branch");
  ...
  assert.ok(!branches.includes("sdd/alpha/M001/S01"), "slice branch cleaned up");
  ...
  assert.deepStrictEqual(getCurrentBranch(wt.path), "sdd/alpha/M001/S02", "on S02 namespaced branch");
  ...
  assert.deepStrictEqual(getCurrentBranch(wt2.path), "sdd/beta/M001/S01", "beta has its own namespaced branch");
  ...
  assert.deepStrictEqual(getCurrentBranch(wt.path), "sdd/alpha/M001/S01", "alpha re-created S01");

  // Both exist simultaneously
  const allBranches = run("git branch", base);
  assert.ok(allBranches.includes("sdd/alpha/M001/S01"), "alpha S01 branch exists");
  assert.ok(allBranches.includes("sdd/beta/M001/S01"), "beta S01 branch exists");
