...
describe('worktree', async () => {
  ...
  console.log("\n=== getSliceBranchName ===");
  assert.deepStrictEqual(getSliceBranchName("M001", "S01"), "sdd/M001/S01", "branch name format correct");
  assert.deepStrictEqual(getSliceBranchName("M001", "S01", null), "sdd/M001/S01", "null worktree = plain branch");
  assert.deepStrictEqual(getSliceBranchName("M001", "S01", "my-wt"), "sdd/my-wt/M001/S01", "worktree-namespaced branch");
  ...
  console.log("\n=== SLICE_BRANCH_RE ===");
  assert.ok(SLICE_BRANCH_RE.test("sdd/M001/S01"), "regex matches plain branch");
  assert.ok(SLICE_BRANCH_RE.test("sdd/my-wt/M001/S01"), "regex matches worktree branch");
  assert.ok(!SLICE_BRANCH_RE.test("main"), "regex rejects main");
  assert.ok(!SLICE_BRANCH_RE.test("sdd/"), "regex rejects bare sdd/");
  ...
  assert.deepStrictEqual(detectWorktreeName("/projects/myapp/.sdd/worktrees/feature-auth"), "feature-auth", "detects worktree name");
  assert.deepStrictEqual(detectWorktreeName("/projects/myapp/.sdd/worktrees/my-wt/subdir"), "my-wt", "detects worktree with subdir");
