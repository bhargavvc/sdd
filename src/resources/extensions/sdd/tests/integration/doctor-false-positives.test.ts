...
function makeBase(): { base: string; sdd: string; mDir: string } {
  const base = mkdtempSync(join(tmpdir(), "sdd-doctor-fp-"));
  const sdd = join(base, ".sdd");
  const mDir = join(sdd, "milestones", "M001");
  mkdirSync(join(mDir, "slices"), { recursive: true });
  return { base, sdd, mDir };
}
...
describe('doctor false-positives (#3105)', async () => {
  ...
  test('Bug 1: orphaned worktree check ignores dirs containing only .sdd/doctor-history.jsonl', async () => {
    // Simulate: a worktree dir that only contains .sdd/doctor-history.jsonl
    // (created by appendDoctorHistory writing to the worktree-scoped path).
    // The orphan check should NOT warn about this directory.
    const { base, sdd } = makeBase();
    writeRoadmap(join(sdd, "milestones", "M001"), `# M001: Test\n\n## Slices\n- [ ] **S01: Slice** \`risk:low\` \`depends:[]\`\n  > After this: done\n`);
    writeSlice(join(sdd, "milestones", "M001"), "S01", "# S01: Slice\n\n**Goal:** G\n**Demo:** D\n\n## Tasks\n- [ ] **T01: Task** `est:10m`\n  Pending.\n");

    // Create a worktree directory that only has .sdd/doctor-history.jsonl
    const wtDir = join(sdd, "worktrees", "M042");
    const wtSddDir = join(wtDir, ".sdd");
    mkdirSync(wtSddDir, { recursive: true });
    writeFileSync(join(wtSddDir, "doctor-history.jsonl"), '{"ts":"2026-01-01","ok":true}\n');

    const result = await runSDDDoctor(base, { fix: false });
    ...
    assert.equal(orphanIssues.length, 0,
      "should not warn about worktree dir that only contains .sdd/doctor-history.jsonl");
