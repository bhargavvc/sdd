...
describe('git-service', async () => {
  ...
  test('GitServiceImpl: smart staging', () => {
    ...
    assert.ok(!showStat.includes(".sdd/activity"), ".sdd/activity/ excluded from commit");
    assert.ok(!showStat.includes(".sdd/runtime"), ".sdd/runtime/ excluded from commit");
    assert.ok(!showStat.includes("STATE.md"), ".sdd/STATE.md excluded from commit");
    assert.ok(!showStat.includes("auto.lock"), ".sdd/auto.lock excluded from commit");
    assert.ok(!showStat.includes("metrics.json"), ".sdd/metrics.json excluded from commit");
    assert.ok(!showStat.includes(".sdd/worktrees"), ".sdd/worktrees/ excluded from commit");
    ...
    assert.ok(statusOut.includes(".sdd/activity/"), "activity still untracked after commit");
    assert.ok(statusOut.includes(".sdd/runtime/"), "runtime still untracked after commit");
    assert.ok(statusOut.includes(".sdd/STATE.md"), "STATE.md still untracked after commit");
  ...
  test('GitServiceImpl: smart staging excludes tracked runtime files', () => {
    ...
    assert.deepStrictEqual(trackedAfter, "", "no .sdd/ runtime files remain in the index");
  ...
  test('GitServiceImpl: autoCommit with extraExclusions', () => {
    ...
    const show = run("git show --stat HEAD", repo);
    assert.ok(!show.includes("ROADMAP"), ".sdd/ files excluded from pre-switch auto-commit");
    assert.ok(show.includes("feature.ts"), "non-.sdd/ files included in pre-switch auto-commit");
  ...
  test('GitServiceImpl: autoCommit extraExclusions — only .sdd/ dirty', () => {
    const repo = initTempRepo();
    const svc = new GitServiceImpl(repo);
    ...
    assert.deepStrictEqual(result, null, "autoCommit returns null when only .sdd/ files are dirty and excluded");
  ...
  test('createSnapshot: enabled by default when prefs omitted', () => {
    ...
    assert.ok(refs.includes("refs/sdd/snapshots/sdd/M001/S01/"), "snapshot ref created under refs/sdd/snapshots/");
  ...
  test('smartStage excludes runtime files, allows milestone artifacts', () => {
    ...
    assert.ok(!committed.includes(".sdd/STATE.md"), "smartStage: STATE.md excluded (runtime)");
    assert.ok(!committed.includes(".sdd/runtime/"), "smartStage: runtime/ excluded");
    assert.ok(!committed.includes(".sdd/activity/"), "smartStage: activity/ excluded");
    // Milestone artifacts SHOULD be committed when not gitignored (#1326)
    assert.ok(committed.includes(".sdd/milestones/"), "smartStage: milestone artifacts ARE committed");
  ...
  test('autoCommit: symlinked .sdd does NOT stage milestone artifacts (#2247)', () => {
    ...
    assert.ok(!committed.includes(".sdd/milestones/"),
