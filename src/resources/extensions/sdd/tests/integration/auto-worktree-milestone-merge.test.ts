...
describe("auto-worktree-milestone-merge", { timeout: 300_000 }, () => {
  ...
  test("auto-resolve .sdd/ state file conflicts", () => {
    const repo = freshRepo();
    ...
    assert.ok(!threw, "auto-resolves .sdd/ state file conflicts without throwing");
  ...
  test("#1738 bug 3: synced .sdd/ dirs cleaned before merge", () => {
    const repo = freshRepo();
    ...
    assert.ok(!threw, "#1738 merge does not fail on synced .sdd/ files");
  ...
  test("#1906: codeFilesChanged=false when only .sdd/ metadata merged", () => {
    const repo = freshRepo();
