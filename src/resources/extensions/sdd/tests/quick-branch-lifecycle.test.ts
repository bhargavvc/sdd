...
async function main(): Promise<void> {
  ...
  assertTrue(QUICK_BRANCH_RE.test("sdd/quick/1-fix-typo"), "matches standard quick branch");
  assertTrue(QUICK_BRANCH_RE.test("sdd/quick/42-some-long-slug-name"), "matches multi-digit quick branch");
  assertTrue(!QUICK_BRANCH_RE.test("main"), "rejects main");
  assertTrue(!QUICK_BRANCH_RE.test("sdd/M001/S01"), "rejects slice branch");
  assertTrue(!QUICK_BRANCH_RE.test("sdd/quickly-something"), "rejects non-quick prefix");
  assertTrue(!QUICK_BRANCH_RE.test("feature/sdd/quick/1"), "rejects nested prefix");
...

  {
    ...
    assertEq(getCurrentBranch(repo), "sdd/quick/1-fix-typo", "on quick branch");
...

  {
    ...
    assertTrue(!branches.includes("sdd/quick/1-fix-typo"), "quick branch deleted");
