...
const CLONE_FIXTURES_ENABLED = process.env.SDD_TEST_CLONE_MARKETPLACES === '1';
...
export function getMarketplaceFixtures(testFileDir: string): { available: boolean; skipReason?: string; fixtures?: MarketplaceFixtureSet } {
  const sdd2Root = resolve(testFileDir, '../../../../..');
  const localClaudeSkillsPath = resolve(sdd2Root, '../claude_skills');
  const localClaudePluginsOfficialPath = resolve(sdd2Root, '../claude-plugins-official');
  ...
  if (!CLONE_FIXTURES_ENABLED) {
    return {
      available: false,
      skipReason: 'Marketplace repos absent and clone-based fixtures are disabled (set SDD_TEST_CLONE_MARKETPLACES=1 to enable)',
    };
  }
  ...
  try {
    const fixtureRoot = mkdtempSync(join(tmpdir(), 'sdd-marketplace-fixtures-'));
