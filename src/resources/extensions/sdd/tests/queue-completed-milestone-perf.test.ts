/**
 * Regression test for #2379: /sdd queue fails with 429 rate limit on projects
...
const tmpBase = mkdtempSync(join(tmpdir(), "sdd-queue-perf-"));
...
for (let i = 1; i <= COMPLETED_COUNT; i++) {
  ...
  mkdirSync(join(sdd, "milestones", mid), { recursive: true });
  writeFileSync(
    join(sdd, "milestones", mid, `${mid}-CONTEXT.md`),
    `# ${mid}: Completed milestone ${i}\n\nThis is a large context document for ${mid}.\n${"Lorem ipsum dolor sit amet. ".repeat(50)}\n`,
  );
  writeFileSync(
    join(sdd, "milestones", mid, `${mid}-SUMMARY.md`),
...
// Create 1 active milestone
{
  ...
  writeFileSync(
    join(sdd, "milestones", mid, `${mid}-ROADMAP.md`),
    `# ${mid} Roadmap\n\nSlices planned.\n`,
  );
}
