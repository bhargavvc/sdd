...
import type { SDDPreferences } from '../preferences.ts';
...
async function main(): Promise<void> {
  ...
  // (h) Preferences round-trip via renderPreferencesForSystemPrompt
  {
    ...
    const prefsTrue: SDDPreferences = { unique_milestone_ids: true };
    ...
    const prefsUndefined: SDDPreferences = {};
    ...
    const prefsFalse: SDDPreferences = { unique_milestone_ids: false };
    ...
    const prefsCoerced: SDDPreferences = { unique_milestone_ids: 1 as unknown as boolean };
    ...
    // SDDPreferences interface accepts the field (compile-time check — if this compiles, it works)
    const prefs: SDDPreferences = { unique_milestone_ids: true, version: 1 };
    assertTrue(prefs.unique_milestone_ids === true, 'SDDPreferences interface accepts unique_milestone_ids');
  }
