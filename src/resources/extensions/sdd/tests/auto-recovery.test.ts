...
test("buildLoopRemediationSteps returns steps for execute-task", () => {
  ...
  try {
    ...
    assert.ok(steps!.includes("sdd doctor"));
...
test("selfHealRuntimeRecords clears stale dispatched records (#769)", async () => {
  ...
  try {
    ...
    // Write a stale runtime record in the worktree .sdd/runtime/units/
...
test("hasImplementationArtifacts returns false when only .sdd/ files committed (#1703)", () => {
...
test("verifyExpectedArtifact complete-milestone fails with only .sdd/ files (#1703)", () => {
  const base = makeGitBase();
  try {
