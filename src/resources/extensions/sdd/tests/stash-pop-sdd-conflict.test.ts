/**
 * stash-pop-sdd-conflict.test.ts — Regression test for #2766.
 *
 * When a squash merge stash-pops and hits conflicts on .sdd/ state files,
 * the UU entries block every subsequent merge. This test verifies that
 * mergeMilestoneToMain auto-resolves .sdd/ conflicts by accepting HEAD
...
function createTempRepo(): string {
  ...
  writeFileSync(join(dir, ".sdd", "STATE.md"), "version: 1\n");
  run("git add .", dir);
...
test("#2766: stash pop conflict on .sdd/ files is auto-resolved", () => {
  ...
  try {
    ...
    // Modify .sdd/STATE.md on the milestone branch (diverges from main)
    writeFileSync(join(wtPath, ".sdd", "STATE.md"), "version: 2-milestone\n");
    run("git add .", wtPath);
    ...
    // Dirty .sdd/STATE.md in the main repo (stash will conflict on pop)
    writeFileSync(join(repo, ".sdd", "STATE.md"), "version: 2-main-dirty\n");

    const roadmap = makeRoadmap("M300", "Stash pop conflict test", [
      { id: "S01", title: "Feature" },
    ]);

    // mergeMilestoneToMain should succeed — .sdd/ conflict auto-resolved
    const result = mergeMilestoneToMain(repo, "M300", roadmap);
    assert.ok(
      result.commitMessage.includes("SDD-Milestone: M300"),
      "merge succeeds despite stash pop conflict on .sdd/ file",
    );
    ...
    assert.strictEqual(stashList, "", "stash is empty after .sdd/ conflict auto-resolution");
  } finally {
    try { rmSync(repo, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 }); } catch { /* cleanup best-effort */ }
  }
});

test("#2766: stash pop conflict on non-.sdd files preserves stash for manual resolution", () => {
  const repo = createTempRepo();
  try {
    ...
    // and is NOT a .sdd/ file, so it should be left for manual resolution
    writeFileSync(join(repo, "README.md"), "# locally modified\n");

    const roadmap = makeRoadmap("M301", "Non-sdd stash conflict", [
    ...
    assert.ok(
      result.commitMessage.includes("SDD-Milestone: M301"),
      "merge succeeds even with non-.sdd stash pop conflict",
    );
  } finally {
    try { rmSync(repo, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 }); } catch { /* cleanup best-effort */ }
  }
