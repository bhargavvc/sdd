...
Add the \`completing-milestone\` phase to the SDD state machine.
...
- \`agent/extensions/sdd/types.ts\` — Phase type definition

## Expected Output

- \`agent/extensions/sdd/types.ts\` — Phase union includes \`'completing-milestone'\`
`;
  const result = parseTaskPlanMustHaves(content);
