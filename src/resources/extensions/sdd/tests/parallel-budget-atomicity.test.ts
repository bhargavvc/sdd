...
function makeTmpBase(): string {
  const base = join(tmpdir(), `sdd-budget-test-${randomUUID()}`);
...
function makePrefs(ceiling?: number): SDDPreferences {
  return {
    parallel: {
      enabled: true,
      max_workers: 2,
...
function writeWorkerCost(
  ...
  writeSessionStatus(base, {
    ...
    worktreePath: join(base, ".sdd", "worktrees", milestoneId.toLowerCase()),
  });
}
