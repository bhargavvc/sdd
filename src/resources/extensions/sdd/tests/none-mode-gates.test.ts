 ...
 * prefs to the runner's cwd .sdd/PREFERENCES.md and clean up in finally.
 */
...
const RUNNER_PREFS_PATH = join(process.cwd(), ".sdd", "PREFERENCES.md");

function writeRunnerPreferences(isolation: "none" | "worktree" | "branch"): void {
  mkdirSync(join(process.cwd(), ".sdd"), { recursive: true });
...
// Worktree isolation requires explicit opt-in — default is "none" so SDD
// works out of the box without PREFERENCES.md (#2480).
// Skip if global prefs exist — they override the default and this test
// cannot control ~/.sdd/PREFERENCES.md.

test('shouldUseWorktreeIsolation returns false for no prefs (default: none)', () => {
  const globalPrefsExist = existsSync(join(homedir(), ".sdd", "PREFERENCES.md"))
    || existsSync(join(homedir(), ".sdd", "PREFERENCES.md"));
  if (!globalPrefsExist) {
    try {
