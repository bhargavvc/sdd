...
async function main(): Promise<void> {
  ...
  {
    const base = mkdtempSync(join(tmpdir(), "sdd-doctor-1850-"));
...
`);
    ...
    // ── First pass: diagnose ──
    const diagReport = await runSDDDoctor(base, { fix: false });
    ...
    // ── Second pass: fix ──
    const fixReport = await runSDDDoctor(base, { fix: true });
    ...
    const rerunReport = await runSDDDoctor(base, { fix: false });
  ...
  {
    const base = mkdtempSync(join(tmpdir(), "sdd-doctor-1850-partial-"));
    const sdd = join(base, ".sdd");
    const mDir = join(sdd, "milestones", "M001");
    const sDir = join(mDir, "slices", "S01");
    const tDir = join(sDir, "tasks");
