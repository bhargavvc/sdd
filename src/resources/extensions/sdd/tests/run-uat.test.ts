...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'sdd-run-uat-test-'));
...
test('(k) run-uat prompt template', () => {
  ...
  const uatResultPath = '.sdd/milestones/M001/slices/S01/S01-UAT.md';
...
test('(k2) run-uat prompt references sdd_summary_save, not direct write', () => {
  const promptResult = loadPromptFromWorktree('run-uat', {
    workingDirectory: '/tmp/test-project',
    milestoneId: 'M001',
    sliceId: 'S01',
    uatPath: '.sdd/milestones/M001/slices/S01/S01-UAT.md',
    uatResultPath: '.sdd/milestones/M001/slices/S01/S01-UAT.md',
    uatType: 'artifact-driven',
    inlinedContext: '<!-- no context -->',
  });

  assert.ok(
    promptResult.includes('sdd_summary_save'),
    'run-uat prompt should reference sdd_summary_save tool',
  );
  assert.ok(
...
test('(m) non-artifact UAT skip', async () => {
    const base = createFixtureBase();
    try {
      const roadmapDir = join(base, '.sdd', 'milestones', 'M001');
...
test('(q) verdict in ASSESSMENT file skips UAT dispatch (file-based path)', async () => {
    ...
    // S{sid}-ASSESSMENT.md (via sdd_summary_save artifact_type:"ASSESSMENT"),
