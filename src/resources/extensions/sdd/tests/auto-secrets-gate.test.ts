 ...
 *
 * Uses temp directories with real .sdd/milestones/M001/ structure, mirroring
...
test('secrets gate: pending keys exist — gate triggers collection, manifest updated on disk', async (t) => {
  ...
  const savedA = process.env.SDD_GATE_TEST_EXISTING;
  t.after(() => {
    delete process.env.SDD_GATE_TEST_EXISTING;
    if (savedA !== undefined) process.env.SDD_GATE_TEST_EXISTING = savedA;
    delete process.env.SDD_GATE_TEST_PEND_A;
    delete process.env.SDD_GATE_TEST_PEND_B;
    rmSync(tmp, { recursive: true, force: true });
  });

  // Simulate one key already in env
  process.env.SDD_GATE_TEST_EXISTING = 'already-here';
...
`);
  ...
  assert.deepStrictEqual(status!.pending, ['SDD_GATE_TEST_PEND_A', 'SDD_GATE_TEST_PEND_B'], 'pending keys');
  assert.deepStrictEqual(status!.existing, ['SDD_GATE_TEST_EXISTING'], 'existing keys');
  ...
  assert.ok(result.skipped.includes('SDD_GATE_TEST_PEND_A'), 'PEND_A should be skipped');
  assert.ok(result.skipped.includes('SDD_GATE_TEST_PEND_B'), 'PEND_B should be skipped');
  assert.deepStrictEqual(result.existingSkipped, ['SDD_GATE_TEST_EXISTING']);
  ...
  const manifestPath = join(tmp, '.sdd', 'milestones', 'M001', 'M001-SECRETS.md');
...
test('secrets gate: no pending keys — getManifestStatus shows pending.length === 0', async (t) => {
  ...
  const savedKey = process.env.SDD_GATE_TEST_ENVKEY;
  t.after(() => {
    delete process.env.SDD_GATE_TEST_ENVKEY;
    if (savedKey !== undefined) process.env.SDD_GATE_TEST_ENVKEY = savedKey;
    rmSync(tmp, { recursive: true, force: true });
  });

  process.env.SDD_GATE_TEST_ENVKEY = 'some-value';
...
`);
  ...
  assert.deepStrictEqual(result!.existing, ['SDD_GATE_TEST_ENVKEY']);
});
