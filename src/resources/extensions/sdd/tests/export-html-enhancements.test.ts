...
test("Feature 10: collapsible sections JS is included", () => {
  ...
  assert.ok(html.includes("sdd-collapsed"), "should reference localStorage key for collapsed state");
});
...
test("Feature 11: dark/light theme toggle JS is included", () => {
  ...
  assert.ok(html.includes("sdd-theme"), "should reference localStorage key for theme");
