...
`;
  fs.writeFileSync(path.join(sddDir, 'DECISIONS.md'), decisionsContent);

  // Verify no DB file exists yet
  const dbPath = path.join(sddDir, 'sdd.db');
  ...
  try {
    ...
    assertTrue(result === true, 'ensureDbOpen should return true when .sdd/ has Markdown');
...
console.log('\n── ensureDbOpen: no .sdd/ returns false ──');

{
  ...
  try {
    const { ensureDbOpen } = await import('../bootstrap/dynamic-tools.ts');
    const result = await ensureDbOpen();
    assertTrue(result === false, 'ensureDbOpen should return false when no .sdd/ exists');
...
console.log('\n── ensureDbOpen: empty .sdd/ returns false ──');

{
  ...
  try {
    const { ensureDbOpen } = await import('../bootstrap/dynamic-tools.ts');
    const result = await ensureDbOpen();
    assertTrue(result === false, 'ensureDbOpen should return false for empty .sdd/');
  } finally {
    process.cwd = origCwd;
    cleanupDir(tmpDir);
  }
