...
test("syncWorktreeStateBack copies task summaries from tasks/ subdirectory (#1678)", () => {
  ...
  try {
    ...
    writeFile(wtBase, `.sdd/milestones/${mid}/${mid}-ROADMAP.md`, "# Roadmap\n");
    writeFile(wtBase, `.sdd/milestones/${mid}/${mid}-SUMMARY.md`, "# Summary\n");
    writeFile(wtBase, `.sdd/milestones/${mid}/slices/S01/S01-PLAN.md`, "# Plan\n");
    writeFile(wtBase, `.sdd/milestones/${mid}/slices/S01/S01-SUMMARY.md`, "# Slice Summary\n");
    writeFile(wtBase, `.sdd/milestones/${mid}/slices/S01/S01-UAT.md`, "# UAT\n");
    writeFile(wtBase, `.sdd/milestones/${mid}/slices/S01/tasks/T01-PLAN.md`, "# Task 1 Plan\n");
    writeFile(wtBase, `.sdd/milestones/${mid}/slices/S01/tasks/T01-SUMMARY.md`, "# Task 1 Summary\n");
    ...
    assert.ok(
      existsSync(join(mainBase, `.sdd/milestones/${mid}/${mid}-ROADMAP.md`)),
      "ROADMAP should be synced",
    );
    assert.ok(
      existsSync(join(mainBase, `.sdd/milestones/${mid}/${mid}-SUMMARY.md`)),
      "SUMMARY should be synced",
    );

    // Verify slice-level files synced
    assert.ok(
      existsSync(join(mainBase, `.sdd/milestones/${mid}/slices/S01/S01-PLAN.md`)),
      "S01-PLAN should be synced",
    );
    assert.ok(
      existsSync(join(mainBase, `.sdd/milestones/${mid}/slices/S01/S01-SUMMARY.md`)),
      "S01-SUMMARY should be synced",
    );

    // Verify task-level files synced (THE BUG FIX)
    assert.ok(
      existsSync(join(mainBase, `.sdd/milestones/${mid}/slices/S01/tasks/T01-PLAN.md`)),
      "T01-PLAN should be synced (was dropped before fix)",
    );
    assert.ok(
      existsSync(join(mainBase, `.sdd/milestones/${mid}/slices/S01/tasks/T01-SUMMARY.md`)),
      "T01-SUMMARY should be synced (was dropped before fix)",
    );
    assert.ok(
      existsSync(join(mainBase, `.sdd/milestones/${mid}/slices/S01/tasks/T02-PLAN.md`)),
      "T02-PLAN should be synced (was dropped before fix)",
    );
    assert.ok(
      existsSync(join(mainBase, `.sdd/milestones/${mid}/slices/S01/tasks/T02-SUMMARY.md`)),
    ...
    const t1Summary = readFileSync(
      join(mainBase, `.sdd/milestones/${mid}/slices/S01/tasks/T01-SUMMARY.md`),
      "utf-8",
    );
...
test("syncWorktreeStateBack handles multiple slices with tasks (#1678)", () => {
  ...
  try {
    ...
    writeFile(wtBase, `.sdd/milestones/${mid}/slices/S01/S01-SUMMARY.md`, "# S01\n");
    writeFile(wtBase, `.sdd/milestones/${mid}/slices/S01/tasks/T01-SUMMARY.md`, "# S01-T01\n");
    writeFile(wtBase, `.sdd/milestones/${mid}/slices/S02/S02-SUMMARY.md`, "# S02\n");
    writeFile(wtBase, `.sdd/milestones/${mid}/slices/S02/tasks/T01-SUMMARY.md`, "# S02-T01\n");
    ...
    assert.ok(existsSync(join(mainBase, `.sdd/milestones/${mid}/slices/S01/tasks/T01-SUMMARY.md`)));
    assert.ok(existsSync(join(mainBase, `.sdd/milestones/${mid}/slices/S02/tasks/T01-SUMMARY.md`)));
    assert.ok(existsSync(join(mainBase, `.sdd/milestones/${mid}/slices/S02/tasks/T02-SUMMARY.md`)));
    assert.ok(existsSync(join(mainBase, `.sdd/milestones/${mid}/slices/S02/tasks/T03-SUMMARY.md`)));

    // Verify content integrity across slices
    assert.equal(
      readFileSync(join(mainBase, `.sdd/milestones/${mid}/slices/S02/tasks/T03-SUMMARY.md`), "utf-8"),
      "# S02-T03\n",
    );
  } finally {
    cleanup(mainBase, wtBase);
...
test("syncWorktreeStateBack handles slices without tasks/ directory", () => {
  ...
  try {
    ...
    assert.ok(existsSync(join(mainBase, `.sdd/milestones/${mid}/slices/S01/S01-RESEARCH.md`)));
...
test("syncWorktreeStateBack ignores non-md files in tasks/", () => {
  ...
  try {
    writeFile(wtBase, `.sdd/milestones/${mid}/slices/S01/S01-PLAN.md`, "# Plan\n");
    writeFile(wtBase, `.sdd/milestones/${mid}/slices/S01/tasks/T01-SUMMARY.md`, "# T01\n");
    ...
    assert.ok(existsSync(join(mainBase, `.sdd/milestones/${mid}/slices/S01/tasks/T01-SUMMARY.md`)));
    assert.ok(!existsSync(join(mainBase, `.sdd/milestones/${mid}/slices/S01/tasks/.DS_Store`)));
    assert.ok(!existsSync(join(mainBase, `.sdd/milestones/${mid}/slices/S01/tasks/notes.txt`)));
  } finally {
    cleanup(mainBase, wtBase);
  }
});
