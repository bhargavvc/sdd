 ...
 *
 * Verifies that `runSDDDoctor({ fix: true })` sanitizes em/en dashes
...
test("doctor fix=true sanitizes em-dash in milestone title", async (t) => {
  const tmpBase = mkdtempSync(join(tmpdir(), "sdd-doctor-delim-"));
...
test("doctor fix=false still reports delimiter_in_title as warning", async (t) => {
  const tmpBase = mkdtempSync(join(tmpdir(), "sdd-doctor-delim-nf-"));
  const sdd = join(tmpBase, ".sdd");
  const mDir = join(sdd, "milestones", "M001");
  const sDir = join(mDir, "slices", "S01");
  const tDir = join(sDir, "tasks");
