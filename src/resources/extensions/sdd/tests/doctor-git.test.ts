...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (process.platform !== "win32") {
    console.log("\n=== worktree_directory_orphaned (symlinked .sdd not false-positive) ===");
    {
      const dir = createRepoWithActiveMilestone();
      cleanups.push(dir);
      ...
      const detect = await runSDDDoctor(dir);
      const orphanDirIssues = detect.issues.filter(i => i.code === "worktree_directory_orphaned");
      assertEq(orphanDirIssues.length, 0, "registered worktree via symlinked .sdd NOT flagged as orphaned");
    }
    } else {
      console.log("\n=== worktree_directory_orphaned (symlinked .sdd — skipped on Windows) ===");
    }
    ...
    console.log("\n=== legacy_slice_branches (fixable) ===");
    {
      ...
      assertEq(remaining, "sdd/quick/1-fix-typo", "quick branch preserved; legacy branches removed");
    }
    } else {
