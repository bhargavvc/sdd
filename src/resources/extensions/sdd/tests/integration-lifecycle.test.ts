...
async function main(): Promise<void> {
  ...
  console.log('\n=== integration-lifecycle: full pipeline ===');
  {
    ...
    mkdirSync(join(sddDir, 'milestones', 'M001'), { recursive: true });
    mkdirSync(join(sddDir, 'milestones', 'M002'), { recursive: true });
    ...
    writeFileSync(join(sddDir, 'DECISIONS.md'), decisionsMarkdown);
    writeFileSync(join(sddDir, 'REQUIREMENTS.md'), requirementsMarkdown);
    writeFileSync(join(sddDir, 'milestones', 'M001', 'M001-ROADMAP.md'), ROADMAP_CONTENT);

    const dbPath = join(sddDir, 'test-lifecycle.db');
    ...
    try {
      ...
      const fullDecisionsContent = readFileSync(join(sddDir, 'DECISIONS.md'), 'utf-8');
      const fullRequirementsContent = readFileSync(join(sddDir, 'REQUIREMENTS.md'), 'utf-8');
      ...
      appendFileSync(join(sddDir, 'DECISIONS.md'), '\n' + newDecisionRow + '\n');
      ...
      const regeneratedMd = readFileSync(join(sddDir, 'DECISIONS.md'), 'utf-8');
