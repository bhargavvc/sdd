 ...
 *   #1245  Stranded .sdd.lock/ directory preventing new sessions
...
async function main(): Promise<void> {
  ...
  {
    const base = mkdtempSync(join(tmpdir(), 'sdd-session-lock-'));
    ...
    try {
      ...
      // The .sdd.lock/ directory should be cleaned up
      const lockDir = sddRoot(base) + '.lock';
      assertTrue(!existsSync(lockDir), '.sdd.lock/ directory removed after release (#1245)');
    } finally {
  ...
  console.log('\n=== 10. re-entrant acquire refreshes lock artifacts ===');
  {
    ...
    try {
      ...
      if (properLockfileAvailable) {
        assertTrue(existsSync(lockDir), '.sdd.lock/ exists after first acquisition');
      }

      const r2 = acquireSessionLock(base);
      assertTrue(r2.acquired, 'second acquisition succeeds');
      if (properLockfileAvailable) {
        assertTrue(existsSync(lockDir), '.sdd.lock/ exists after re-entrant acquisition');
      }
      assertTrue(validateSessionLock(base), 'lock remains valid after re-entrant acquisition');

      releaseSessionLock(base);
      assertTrue(!existsSync(lockDir), '.sdd.lock/ is removed after release');
    } finally {
      rmSync(base, { recursive: true, force: true });
    }
  }
