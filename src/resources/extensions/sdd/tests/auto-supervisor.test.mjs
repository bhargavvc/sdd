...
test('writeUnitRuntimeRecord persists progress and recovery metadata defaults', () => {
  const base = mkdtempSync(join(tmpdir(), 'sdd-auto-supervisor-'));
...
test('writeUnitRuntimeRecord keeps explicit recovery attempt fields', () => {
  ...
  const runtime = JSON.parse(readFileSync(join(base, '.sdd/runtime/units/research-milestone-M011.json'), 'utf8'));
