...
// sdd-recover.test.ts — Tests for the `sdd recover` recovery logic.
...
function createFixtureBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'sdd-recover-'));
...
// ─── Recovery helpers (mirrors sdd recover handler logic) ─────────────────
