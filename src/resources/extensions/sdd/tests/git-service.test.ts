...
async function main(): Promise<void> {
  ...
  assertTrue(
    RUNTIME_EXCLUSION_PATHS.includes(".sdd/activity/"),
    "includes .sdd/activity/"
  );
  assertTrue(
    RUNTIME_EXCLUSION_PATHS.includes(".sdd/STATE.md"),
    "includes .sdd/STATE.md"
  );
...

  {
    ...
    createFile(repo, ".sdd/activity/log.jsonl", "log data");
    createFile(repo, ".sdd/runtime/state.json", '{"state":true}');
    createFile(repo, ".sdd/STATE.md", "# State");
    createFile(repo, ".sdd/auto.lock", "lock");
    createFile(repo, ".sdd/metrics.json", "{}");
    createFile(repo, ".sdd/worktrees/wt/file.txt", "wt data");
    ...
    assertTrue(!showStat.includes(".sdd/activity"), ".sdd/activity/ excluded from commit");
    assertTrue(!showStat.includes(".sdd/runtime"), ".sdd/runtime/ excluded from commit");
    assertTrue(!showStat.includes("STATE.md"), ".sdd/STATE.md excluded from commit");
    assertTrue(!showStat.includes("auto.lock"), ".sdd/auto.lock excluded from commit");
    assertTrue(!showStat.includes("metrics.json"), ".sdd/metrics.json excluded from commit");
    assertTrue(!showStat.includes(".sdd/worktrees"), ".sdd/worktrees/ excluded from commit");

    // Verify runtime files are still untracked
    // git status --short may collapse to "?? .sdd/" or show individual files
    // Use --untracked-files=all to force individual listing
    const statusOut = run("git status --short --untracked-files=all", repo);
    assertTrue(statusOut.includes(".sdd/activity/"), "activity still untracked after commit");
    assertTrue(statusOut.includes(".sdd/runtime/"), "runtime still untracked after commit");
    assertTrue(statusOut.includes(".sdd/STATE.md"), "STATE.md still untracked after commit");
  ...
  {
    // Reproduces the real bug: .sdd/ runtime files that are already tracked
    // (in the git index) must be excluded from staging even when .sdd/ is
    ...
    // 3. Pre-checkout discard: git checkout -- .sdd/ clears dirty runtime files

    const repo = initTempRepo();
    const svc = new GitServiceImpl(repo);

    // Simulate a repo where .sdd/ files were previously force-added
    createFile(repo, ".sdd/metrics.json", '{"version":1}');
    createFile(repo, ".sdd/completed-units.json", '["unit1"]');
    createFile(repo, ".sdd/activity/log.jsonl", '{"ts":1}');
    createFile(repo, "src/real.ts", "real code");
    // Force-add .sdd/ files to simulate historical tracking
    runGit(repo, ["add", "-f", ".sdd/metrics.json", ".sdd/completed-units.json", ".sdd/activity/log.jsonl", "src/real.ts"]);
    runGit(repo, ["commit", "-F", "-"], { input: "init with tracked runtime files" });

    // Add .gitignore with .sdd/ (matches real-world setup from ensureGitignore)
    createFile(repo, ".gitignore", ".sdd/\n");
    runGit(repo, ["add", ".gitignore"]);
    runGit(repo, ["commit", "-F", "-"], { input: "add gitignore" });

    // Verify runtime files are tracked (precondition)
    const tracked = run("git ls-files .sdd/", repo);
    ...
    createFile(repo, ".sdd/metrics.json", '{"version":2}');
    createFile(repo, ".sdd/completed-units.json", '["unit1","unit2"]');
    createFile(repo, ".sdd/activity/log.jsonl", '{"ts":2}');
    ...
    const trackedAfter = run("git ls-files .sdd/", repo);
    assertEq(trackedAfter, "", "no .sdd/ runtime files remain in the index");

    // Verify a second autoCommit with changed runtime files does NOT stage them
    createFile(repo, ".sdd/metrics.json", '{"version":3}');
    createFile(repo, ".sdd/completed-units.json", '["unit1","unit2","unit3"]');
...

  {
    ...
    createFile(repo, ".sdd/activity/x.jsonl", "data");
...

  {
    ...
    // Create both a .sdd/ planning file and a regular source file
    ...
    assertEq(msg, "chore(main): auto-commit after pre-switch", "pre-switch autoCommit with .sdd/ exclusion commits");

    // Verify .sdd/ file was NOT committed
    const show = run("git show --stat HEAD", repo);
    assertTrue(!show.includes("ROADMAP"), ".sdd/ files excluded from pre-switch auto-commit");
    assertTrue(show.includes("feature.ts"), "non-.sdd/ files included in pre-switch auto-commit");

    rmSync(repo, { recursive: true, force: true });
  }

  // ─── GitServiceImpl: autoCommit extraExclusions — only .sdd/ dirty ────
...

  {
    ...
    // Create only .sdd/ planning files
    createFile(repo, ".sdd/milestones/M001/M001-ROADMAP.md", "- [x] S01");
    createFile(repo, ".sdd/STATE.md", "state content");

    // Auto-commit with .sdd/ excluded — nothing else to commit
    const result = svc.autoCommit("pre-switch", "main", [".sdd/"]);
    assertEq(result, null, "autoCommit returns null when only .sdd/ files are dirty and excluded");
...

  {
    ...
    assertEq(svc.getCurrentBranch(), "sdd/M001/S01", "getCurrentBranch returns slice branch name");
...

  {
    ...
    assertTrue(refs.includes("refs/sdd/snapshots/sdd/M001/S01/"), "snapshot ref created under refs/sdd/snapshots/");
...

  {
    ...
    assertTrue(existsSync(join(repo, ".sdd", "completed-units.json")),
      "untrack: completed-units.json still on disk");
    assertTrue(existsSync(join(repo, ".sdd", "metrics.json")),
...

  {
    ...
    assertTrue(!committed.includes(".sdd/STATE.md"), "smartStage: STATE.md excluded (runtime)");
    assertTrue(!committed.includes(".sdd/runtime/"), "smartStage: runtime/ excluded");
    assertTrue(!committed.includes(".sdd/activity/"), "smartStage: activity/ excluded");
    // Milestone artifacts SHOULD be committed when not gitignored (#1326)
    assertTrue(committed.includes(".sdd/milestones/"), "smartStage: milestone artifacts ARE committed");
...

  {
    ...
    // No commit — .sdd/ is managed externally
  ...
  console.log("\n=== ensureGitignore: adds .sdd entry ===");

  {
    ...
    assertTrue(lines.includes(".sdd"), "ensureGitignore: .gitignore contains .sdd");
...

  {
    ...
    assertTrue(!threw, "nativeAddAllWithExclusions does not throw with symlinked .sdd");

    // Verify the real file was staged
    const staged = run("git diff --cached --name-only", repo);
    assertTrue(staged.includes("src/app.ts"), "real file staged despite symlinked .sdd");
    assertTrue(!staged.includes(".sdd"), ".sdd content not staged");
...

  {
    ...
    assertTrue(!threw, "nativeAddAllWithExclusions works with normal .sdd directory");

    const staged = run("git diff --cached --name-only", repo);
    assertTrue(staged.includes("src/code.ts"), "real file staged with normal .sdd");
  ...
  console.log("\n=== MergeConflictError: constructor fields ===");
  {
    ...
    assertEq(err.branch, "sdd/M001/S01", "MergeConflictError.branch set");
  ...
  console.log("\n=== Integration branch: rejects sdd/quick/* branches ===");
  {
    ...
    assertEq(readIntegrationBranch(repo, "M001"), null, "sdd/quick/* branches are not recorded as integration branch");
  ...
  // ─── autoCommit: symlinked .sdd stages new milestone artifacts (#2104) ──

  console.log("\n=== autoCommit: symlinked .sdd stages new milestone artifacts (#2104) ===");

  {
    // Reproduction: when .sdd is a symlink (external state project),
    // autoCommit silently fails to stage NEW .sdd/milestones/ files because:
    //   1. nativeAddAllWithExclusions falls back to plain `git add -A` (symlink)
    //   2. `.sdd` is in .gitignore → new .sdd/ files are invisible to `git add`
    // The fix: smartStage() force-adds .sdd/milestones/ after the normal staging.
    const repo = initTempRepo();
    ...
    assertTrue(committed.includes(".sdd/milestones/M009/M009-SUMMARY.md"),
      "symlink autoCommit: new M009-SUMMARY.md is committed (not silently dropped)");
    assertTrue(committed.includes(".sdd/milestones/M009/S01-SUMMARY.md"),
      "symlink autoCommit: new S01-SUMMARY.md is committed");
    assertTrue(committed.includes(".sdd/milestones/M009/T01-VERIFY.json"),
