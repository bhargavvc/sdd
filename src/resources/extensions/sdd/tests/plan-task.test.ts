...
import { openDatabase, closeDatabase, insertMilestone, insertSlice, insertTask, getTask } from '../sdd-db.ts';
import { handlePlanTask } from '../tools/plan-task.ts';
import { parseTaskPlanFile } from '../files.ts';

function makeTmpBase(): string {
  const base = mkdtempSync(join(tmpdir(), 'sdd-plan-task-'));
...
test('handlePlanTask writes planning state and renders task plan', async () => {
  ...
  try {
    ...
    const taskPlanPath = join(base, '.sdd', 'milestones', 'M001', 'slices', 'S02', 'tasks', 'T02-PLAN.md');
