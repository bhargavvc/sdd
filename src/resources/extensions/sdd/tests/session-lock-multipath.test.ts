...
async function main(): Promise<void> {
  ...
  console.log('\n=== 1. Lock dir registry tracks sddDir on acquisition ===');
  {
    ...
    try {
      ...
      assertTrue(registered.includes(sddDir), 'sddDir is registered in lock dir registry');
  ...
  console.log('\n=== 2. Release cleans lock files at all registered paths ===');
  {
    ...
    try {
      ...
      assertTrue(!existsSync(primaryLockDir), 'primary .sdd.lock/ removed after release');
    } finally {
      rmSync(base, { recursive: true, force: true });
    }
  }
  ...
  console.log('\n=== 3. Re-entrant acquisition registers path once ===');
  {
    ...
    try {
      ...
      assertEq(count, 1, 'sddDir registered exactly once after re-entrant acquisition');
  ...
  console.log('\n=== 4. Multiple base paths all get registered ===');
  {
    ...
    try {
      ...
      assertTrue(registered.includes(sdd2), 'second sddDir is registered');
  ...
  console.log('\n=== 5. Full acquire/release cycle cleans all artifacts ===');
  {
    ...
    try {
      ...
      assertTrue(!existsSync(lockDir), '.sdd.lock/ cleaned');
