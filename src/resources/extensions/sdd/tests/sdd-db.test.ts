...
function tempDbPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'sdd-db-test-'));
...
describe('sdd-db', () => {
  test('sdd-db: provider detection', () => {
  ...
  test('sdd-db: fresh DB schema init (memory)', () => {
  ...
  test('sdd-db: double-init idempotency', () => {
  ...
  test('sdd-db: insert + get decision', () => {
    openDatabase(':memory:');
    insertDecision({
      id: 'D042',
  ...
  test('sdd-db: insert + get requirement', () => {
    openDatabase(':memory:');
    insertRequirement({
      id: 'R007',
      class: 'functional',
  ...
  test('sdd-db: active_decisions view excludes superseded', () => {
    openDatabase(':memory:');
  ...
  test('sdd-db: active_requirements view excludes superseded', () => {
    openDatabase(':memory:');
  ...
  test('sdd-db: WAL mode on file-backed DB', () => {
    const dbPath = tempDbPath();
    openDatabase(dbPath);
  ...
  test('sdd-db: transaction rollback on error', () => {
    openDatabase(':memory:');
  ...
  test('sdd-db: query wrappers return null/empty when DB unavailable', () => {
    // Ensure DB is closed
    closeDatabase();
