...
export async function collectUndoInfo(projectCwdOverride?: string): Promise<UndoInfo> {
  ...
  const sddDir = join(projectCwd, ".sdd")
  const completedPath = join(sddDir, "completed-units.json")
...
export async function executeUndo(projectCwdOverride?: string): Promise<UndoResult> {
  ...
  const undoResolution = resolveSubprocessModule(packageRoot, "resources/extensions/sdd/undo.ts")
  const pathsResolution = resolveSubprocessModule(packageRoot, "resources/extensions/sdd/paths.ts")
  ...
  const script = [
    ...
    'const sddDir = pathsMod.sddRoot(basePath);',
    'const completedPath = join(sddDir, "completed-units.json");',
